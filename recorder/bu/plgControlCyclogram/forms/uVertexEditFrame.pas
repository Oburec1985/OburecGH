...
type
  // управл€ет по тегам движением сцены u3dMoveEngine.g_CtrlObjList
...
begin
  ...
  begin
    ...
    // признак что така€ кость найдена
...
begin
  ...
  begin
    ...
    deformP.weight:=PointWeight.Value;
    SkinPointsLV.SetSubItemByColumnName('¬ес',floattostr(PointWeight.Value),li);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        li:=VertLV.Items.Add;
        VertLV.SetSubItemByColumnName('є',inttostr(n),li);
        p3:=l.data[j];
...
begin
  ...
  begin
    ...
    SkinPointsLV.SetSubItemByColumnName('є',inttostr(i),li);
    SkinPointsLV.SetSubItemByColumnName('ID', tpointtostr(dp.p), li);
    SkinPointsLV.SetSubItemByColumnName('¬ес',floattostr(dp.weight),li);
  end;
