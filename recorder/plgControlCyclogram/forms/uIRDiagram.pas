...
begin
  if fspm.LastBlockTime>xTime then
  begin
...
begin
  ...
  begin
      ...
      startind := round(x * fband.x / fspm.dX);
      endind := round(x * fband.y / fspm.dX);
  ...
  begin
    ...
    res := fband_i.x;
    for I := fband_i.x+1 to fband_i.y - 1 do
    begin
      if fspm.m_rms[i]>res then
      begin
        v:=fspm.cmplx_resArray[i];
      end;
    end;
