...
type
  ...
  public
    ...
    // буфер данных посчитанный по alglib
  ...
  protected
    ...
    // иначе p_blockcount игнорируется и происходит расчет возможного p_blockcount/ числа дополнения нулей
...
begin
  ...
  // в данном алгоритме отбрасывается часть полезных данных если размер порции
  // больше числа точек fft
  begin
...
begin
  ...
  begin
    case m_I of
      3: result:=0; // однократный комплексный спектр
      4: result:=1;  // интеграл комплексный спектр
      5: result:=2; // 2интеграл комплексный спектр
    end;
  end;
end;
...
begin
  ...
  if m_tag = nil then
  begin
    m_errors.add('cSpm.updateOutChan Входной тег не создан:');
    exit;
  end;
  if m_tag.tag = nil then
  begin
