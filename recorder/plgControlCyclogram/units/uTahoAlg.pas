...
begin
  ...
  // результат отличается от FFTAnalyse на первую точку!!!! (т.е. частота возможно
  // врет на один отсчет в dx) 01.09.19
  fft_al_d_sse(TDoubleArray(m_spmArray.p), tCmxArray_d(m_cmxArr.p), FFTPlan);
  Knorm:=1/(FFTPlan.PCount shr 1);
  // для тахо не пересчитываем все точки только максимум в конце кода
