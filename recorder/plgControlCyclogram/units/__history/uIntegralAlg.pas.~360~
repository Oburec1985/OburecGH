...
begin
  ...
  logmessage(logstr);
  m_OutTag.tag.PushDataEx(pointer(m_OutTag.m_ReadData)^, len, 0, m_InTag.m_ReadDataTime);
  m_outFileTag.saveBlock(m_InTag.m_ReadDataTime,len);
end;
...
begin
  ...
  if m_FileTag = nil then
  begin
    m_FileTag := cFileTag.create('FT_'+m_InTag.tagname, m_InTag.tag.getfreq);
  ...
  if m_outFileTag = nil then
  begin
    m_outFileTag := cFileTag.create('FT_out_'+m_InTag.tagname, m_InTag.tag.getfreq);
    /// /p:=addr(m_InTag.m_TagData);
