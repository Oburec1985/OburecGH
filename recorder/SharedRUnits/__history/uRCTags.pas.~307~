...
type
  ...
  public
    ...
    m_lastindex,
    // количество готовых к записи в буфер точек
...
begin
  ...
  begin
    newdata:=true;
    //logmessage('finTag Lastindex='+inttostr(finTag.lastindex));
    //logstr:='finTag.m_ReadData: '+'i1='+inttostr(i1)+';'+'ioverflow='+inttostr(i1+foverflow-1)+'; '
    //                            +'i2='+inttostr(i1+m_FFTCount-1)+'; '+'v1='+formatstr(TDoubleArray(finTag.m_ReadData)[i1],4)+'; '
    //                             +'Voverflow='+formatstr(TDoubleArray(finTag.m_ReadData)[i1+foverflow-1],4)+'; '+'v2='+formatstr(TDoubleArray(finTag.m_ReadData)[i1+m_FFTCount-1],4)+'; '
    //                             +'Time=' +formatstr(finTag.m_ReadDataTime, 4);
    //logmessage(logstr);
    ...
    ind := i1;
    //logstr:='m_outData: '+'v1='+formatstr(m_outData[i1],4)+'; '
    //                     +'Voverflow='+formatstr(m_outData[i1+foverflow-1],4)+'; '
    //                     +'v2='+formatstr(m_outData[m_FFTCount-1],4)+'; '
    //                     +'Time=' +formatstr(finTag.m_ReadDataTime,4);
    //logmessage(logstr);
    // если не первый блок
