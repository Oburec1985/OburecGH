#!/usr/bin/env python3
"""Fix double-spaced lines in uOglChartPage.pas."""
from __future__ import annotations

from pathlib import Path

TARGET = Path(__file__).resolve().parents[1] / "uOglChartPage.pas"

CONTENT = r"""unit uOglChartPage;
...
begin
  ...
  if AIsMin then
  begin
    APage.XMinValue := AValue;
    APage.PresetMinXValue := AValue;
  end
  else
  begin
    APage.XMaxValue := AValue;
    APage.PresetMaxXValue := AValue;
  end;
  if APage.PresetMaxXValue > APage.PresetMinXValue then
    APage.HasPresetXRange := True;
end;
...
def main() -> None:
    ...
    lines = CONTENT.count("\n") + 1
    empty_runs = CONTENT.count("\n\n\n")
    print(f"Written {TARGET} lines={lines} triple-blank={empty_runs}")
