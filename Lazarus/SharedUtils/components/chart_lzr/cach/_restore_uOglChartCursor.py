# -*- coding: utf-8 -*-
"""Restore uOglChartCursor.pas with correct UTF-8 comments."""
from pathlib import Path

CONTENT = r'''unit uOglChartCursor;
{
...
begin
  ...
  fX1 := 0.0;
  fX2 := 0.0;
  fColor := $1E808080; // Очень прозрачный серый
  fPeakX := 0.0;
  fPeakY := 0.0;
  fRms := 0.0;
end;
...
path = Path(__file__).resolve().parents[1] / 'uOglChartCursor.pas'
path.write_bytes(CONTENT.replace('\n', '\r\n').encode('utf-8'))
print('restored', path, 'lines', len(CONTENT.splitlines()))
