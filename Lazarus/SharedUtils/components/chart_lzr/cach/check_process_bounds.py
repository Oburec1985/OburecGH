...
def get_func_lines(content, func_name):
    ...
    for idx, line in enumerate(lines):
        if func_name.lower() in line.lower() and "procedure" in line.lower() and not "forward" in line.lower():
            start = idx
            break
    if start == -1:
        return []
...
if diff:
    print("Mismatch in ProcessObjectBounds:")
    for line in diff:
        print(line)
else:
    print("Match in ProcessObjectBounds")
