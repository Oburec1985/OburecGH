...
begin
  ...
  begin
    ...
    else if (TControl(ASender).Cursor = crSizeNWSE) or (TControl(ASender).Cursor = crSizeWE) or (TControl(ASender).Cursor = crSizeNS) or (TControl(ASender).Cursor = crSizeAll) then
    begin
      TControl(ASender).Cursor := crDefault;
    end;
