...
## 2026-09-04 — восстановлено отвязывание тегов

Пользователь уточнил требуемую семантику: удаление источника не означает
удаление его тегов. Теги должны оставаться в проекте и отображаться красной
иконкой с перечёркнутым кругом, чтобы ссылки компонентов не терялись.

Изменение 2026-08-19, удалявшее теги через `RemoveTagsBySourceId`, отменено на
уровне поведения:

- добавлена единая операция `DetachTagsBySourceId`;
- Mera, MIC-140, MIC-185 и общий аппаратный путь удаления источника теперь
  записывают тегу `SourceId=Detached: <старый source id>`;
- очистка тегов отсутствующих источников тоже отвязывает, а не удаляет их;
- сохранение и загрузка проекта больше не отбрасывают `Detached:`-теги;
- повторное добавление одноимённого канала по-прежнему может заменить старый
  отвязанный тег без ошибки дублирования;
- при возврате того же источника поиск канала учитывает нормализованный
  `SourceId`, снимает `Detached:` и повторно использует тот же тег, имя и ID.

Проект пересобран `lazbuild -B RecorderLnx.lpi` без ошибок.
