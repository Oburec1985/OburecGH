## 2026-07-24 — Запись MERA: ложные .prt из гонки LastBlockSnapshot

**Задача:** Убрать «забор» на графике и лишние `.prt` при нормальном числе точек (20 с → ~2000 @100 Гц).

**Сделано:**
- Причина: UI писал `LastBlockSnapshot` после гонки с источником → дубликаты блоков и ложные разрывы; детект `.prt` был по половине периода отсчёта, а не длительности блока (как в оригинале).
- `PublishBlockNotifications` кладёт полный блок в событие; запись берёт `ASnapshot.Times/Values`, не `LastBlockSnapshot`.
- `.prt`: критерий оригинала `dT > blockDuration/2`; для каналов с явным `.x` авто-порции не пишутся.
- `lazbuild -B` RecorderLnx — OK.

**Документация:** [original-recorder-block-recording.md](Docs/original-recorder-block-recording.md)

## 2026-07-24 — MIC-140: drain за период опроса (без обрезки и фикса 200 мс)

**Задача:** Корректная запись при любом `UpdateTimeMs` (в т.ч. 0.1 с): столько точек, сколько накопилось за период (`≈ freq×dt`), без потерь.

**Сделано:**
- Убраны набор «ровно N» и обрезка порций (они выкидывали отсчёты).
- `DoTick`: один blocking `ReadBlock(UpdateTimeMs)` + осушение кольца `ReadBlock(0)` до пусто.
- `ReadBlock(0)` — неблокирующая проверка кольца.
- Ожидание/лог завязаны на фактический `UpdateTimeMs` источника, без жёсткого 200 мс / 20 точек.
- `lazbuild -B` RecorderLnx — OK.

**Документация:** [mic140_protocol.md § draining](Docs/old/mic140_protocol.md)
