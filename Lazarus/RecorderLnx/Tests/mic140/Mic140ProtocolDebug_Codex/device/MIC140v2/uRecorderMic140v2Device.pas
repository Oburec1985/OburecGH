...
type
  { Профиль оставлен в локальном контракте теста, чтобы UI и драйвер
    эталонного стенда собирались как единый автономный срез. }
  TMic140ProgrammingProfile = (mppMic14048v3);
...
constructor TRecorderMic140v2Device.Create(const ADeviceId, AHost: string;
  APort: Word; AChannelCount: Integer; APollFrequencyHz: Double;
  AUpdateTimeMs: Cardinal; AProfile: TMic140ProgrammingProfile;
  AGroundEnabled: Boolean);
begin
  { Снимок протокола 010 содержит единственный профиль MIC-140-48v3.
    Параметры оставлены явными в UI, но старый драйвер применяет свой
    зафиксированный профиль программирования. }
  Create(ADeviceId, AHost, APort, AChannelCount, APollFrequencyHz,
    AUpdateTimeMs);
  fV3Profile := AProfile = mppMic14048v3;
  fGroundEnabled := AGroundEnabled;
end;

procedure TRecorderMic140v2Device.InitializeDevice;
var
  lError, lResetScanError: string;
  lReply: TMic140v2WordBuf;
begin
  { ВАЖНО ДЛЯ ИНТЕГРАЦИИ В RECORDER:
    холодная инициализация MIC-140 содержит обязательное ожидание загрузки BIOS
    около пяти секунд. Этот метод синхронный, поэтому менеджер устройств должен
    вызывать Connect/Init в служебной задаче, а не из потока UI. Инициализации
    независимых устройств следует запускать параллельно; в UI передавать только
    состояние/прогресс и результат. Config/Play/Stop к разовой загрузке BIOS
    не относятся и не должны повторно вызывать этот холодный Init. }
  if fState = rdsDisconnected then
    Connect;
  if (fState = rdsDisconnected) or (fCli = nil) or fInitialized then
    Exit;

  if not fCli.TestLoad(lError) then
    raise ERecorderDeviceError.Create('MIC-140 TEST_LOAD: ' + lError);
  Mic140v2Log(Format('[MIC140v2:%s:%d] init TEST_LOAD ready',
    [fHost, fPort]));

  if not fCli.ReadFirmware(fFw, lError) then
    raise ERecorderDeviceError.Create('MIC-140 firmware before reset: ' +
      lError);
  fHwSer := Mic140v2HardwareCalibrSerial(fFw);

  if not fCli.CallCommand(CMic140LegacyCmdReset, nil, 0, lReply,
    lError) then
    raise ERecorderDeviceError.Create('MIC-140 CMD_RESET: ' + lError);
  fCli.ClearBufferedPackets;

  Sleep(CMic140LegacyBiosResetFirstDelayMs);
  if not fCli.ReadFirmware(fFw, lError) then
    raise ERecorderDeviceError.Create('MIC-140 firmware after reset: ' +
      lError);
  fHwSer := Mic140v2HardwareCalibrSerial(fFw);

  if not fCli.CallCommand(CMic140LegacyCmdResetScanMain, nil, 0, lReply,
    lResetScanError) then
    raise ERecorderDeviceError.Create('MIC-140 first RESETSCANMAIN: ' +
      lResetScanError);
  ...
  if not fCli.CallCommand(CMic140LegacyCmdResetScanMain, nil, 0, lReply,
    lResetScanError) then
    raise ERecorderDeviceError.Create('MIC-140 second RESETSCANMAIN: ' +
      lResetScanError);
...
begin
  ...
  { При первом Config холодный Init уже удалил и остановил прежний scan.
    Перед повторным Config оригинал сначала удаляет старый scan, затем
    останавливает его. Эти команды не принадлежат построению таблиц. }
...
function TRecorderMic140v2Device.GetNativeObject: TObject;
begin
  Result := Self;
end;
...
function TRecorderMic140v2Device.TestLink(out AErrorText: string): Boolean;
begin
  Result := False;
  AErrorText := '';
