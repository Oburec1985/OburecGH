...
uses
  uRecorderDebugLog, uRecorderMic140v2Consts;
...
begin
  ...
  begin
    if AVerdict.NumBuffOk then
      AVerdict.Detail := Format(
        'OK size=%d scan_id=%d state=0 data=%d num_buff=%d',
        [AVerdict.ActualMessageSize, CMic140LegacyScanId, AActualDataWords,
         lNumBuff])
    else
      AVerdict.Detail := Format(
        'OK size=%d scan_id=%d state=0 data=%d num_buff=%d seq=gap',
        [AVerdict.ActualMessageSize, CMic140LegacyScanId, AActualDataWords,
         lNumBuff]);
  end
  else
...
var
  ...
  lReadyWord: Word;
begin
  { [ORIG] mdpEthernet81::CheckInitialized. Размер и содержимое запроса
    подтверждены дампом первого запуска оригинального Recorder. }
