...
begin
  Result := Mic140v2CodeInRecorderProfile(ACode, AIndex);
end;
...
var
  ...
  keptFirst, originalSamples: Integer;
  desync: Boolean;
  corrupt: Boolean;
...
begin
  ...
  { Потоковая часть не имеет права отбрасывать или сдвигать строки по
    совпадению с сохранёнными когда-то кодами конкретного прибора. Целостность
    определяется размером пакета, stride, checksum и num_buff. Оценка формы
    сигнала остаётся только диагностикой и не изменяет данные. }
  keptFirst := 0;
...
begin
  ...
  SAux.ChannelCount := AStride - AChCnt;
  if SAux.ChannelCount > MIC140v3VisibleTemperatureChannelCount then
    SAux.ChannelCount := MIC140v3VisibleTemperatureChannelCount;
  ...
  begin
    ...
    begin
      ...
      SAux.Valid[i][j] := (AChCnt + i < AStride) and (idx < ARaw.DataWordCount);
