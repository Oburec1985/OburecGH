...
type
  ...
  { TRecorderDevice — базовая реализация IRecorderDevice.
    Наследники (MIC-140 и др.) переопределяют специфику. }
...
begin
  ...
  case AProperty of
    ...
    rdpPort:           fPort := AValue;
    rdpPollFrequencyHz: SetFs(AValue);
    rdpUpdateTimeMs:   fUpdateTimeMs := AValue;
    rdpChannelCount:   fChannelCount := AValue;
  else
    Result := False;
  end;
end;
