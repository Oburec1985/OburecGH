...
{
  Shadow unit: IMic140Device для стенда (только Mic140ProtocolDebug).

  ScanStride берётся из shadow-scan (fifoStride 48 или 51).
  При fifoStride=48 и tin-slots>0 TIn читаются из DM (CMD_READMEMDM=114).
  При fifoStride=51 TIn в payload (слоты 48..50); DM-read не нужен.
}
...
begin
  ...
  try
    ...
    begin
      ...
      fTinSlots := Mic140v2DebugTinSlotCount;
...
begin
  ...
  { stride=48: TIn только в DM; stride=51: TIn уже в SAux из FIFO }
