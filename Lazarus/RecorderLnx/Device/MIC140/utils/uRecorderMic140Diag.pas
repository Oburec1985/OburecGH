unit uRecorderMic140Diag;

{
  ╨Ы╨╛╨│ ╨╕ ╨┐╤А╨╛╨▓╨╡╤А╨║╨░ corrupt raw-╨▒╨╗╨╛╨║╨░.

  [LNX] ╨┐╨╛╤А╨╛╨│╨╕ misalign/saturation тАФ ╤Н╨▓╤А╨╕╤Б╤В╨╕╨║╨░ TCP-╤А╨╕╨┤╨╡╤А╨░, ╨╜╨╡╤В ╨▓ windev BIOS.
}
...
const
  CMic140v2ScanDetailLogBlocks = 15;     { LNX: ╨┐╨╛╨┤╤А╨╛╨▒╨╜╤Л╨╣ ╨╗╨╛╨│ ╨┐╨╡╤А╨▓╤Л╤Е N ╨▒╨╗╨╛╨║╨╛╨▓ }
  CMic140v2NumBuffDesyncMissed = 32;    { LNX: ╨┐╨╛╤А╨╛╨│ num_buff тЖТ stream restart }
...
function Mic140v2RawRowRecorderProfile(const ARaw: TMic140LegacyRawBlock;
  AOffset, AUserCh: Integer): Boolean;
function Mic140v2RecorderReferenceCode(AChannelIndex: Integer; out ACode: Integer): Boolean;
function Mic140v2CodeInRecorderProfile(ACode, AChannelIndex: Integer): Boolean;
...
const
  ...
  CMinHeadNominal = 8;
  CHeadStartWindow = 4;
  CRecorderProfile: array[0..47] of Integer =
    (-7176, -7129, -6605, -7239, -7178, -7127, -6606, -7238,
     -7176, -7130, -6603, -7241, -7176, -7125, -6605, -7237,
     -7175, -7126, -6604, -7239, -7178, -7127, -6605, -7239,
     -16793, -15931, -17766, -17550, 6074, 13499, -12571, -15394,
     -16066, -13795, -16349, -17467, -20524, -17895, -18407, -18592,
     -20674, -17309, -17422, -17725, -19990, -15853, -16980, -19364);
  CRecorderProfileTolerance = 20;
...
function Mic140v2RecorderReferenceCode(AChannelIndex: Integer; out ACode: Integer): Boolean;
begin
  Result := (AChannelIndex >= Low(CRecorderProfile)) and
    (AChannelIndex <= High(CRecorderProfile));
  if Result then
    ACode := CRecorderProfile[AChannelIndex]
  else
    ACode := 0;
end;

function Mic140v2RecorderProfileTolerance(AChannelIndex: Integer): Integer;
begin
  Result := CRecorderProfileTolerance;
end;
...
var
  lExpected: Integer;
begin
  Result := Mic140v2RecorderReferenceCode(AChannelIndex, lExpected) and
    (Abs(ACode - lExpected) <= Mic140v2RecorderProfileTolerance(AChannelIndex));
end;

function Mic140v2RawRowRecorderProfile(const ARaw: TMic140LegacyRawBlock;
  AOffset, AUserCh: Integer): Boolean;
begin
  { ╨в╤А╨░╨╜╤Б╨┐╨╛╤А╤В╨╜╤Л╨╣ ╤Б╨╗╨╛╨╣ ╨┐╤А╨╛╨▓╨╡╤А╤П╨╡╤В ╤В╨╛╨╗╤М╨║╨╛ ╨╜╨░╨╗╨╕╤З╨╕╨╡ ╨┐╨╛╨╗╨╜╨╛╨╣ ╤Б╤В╤А╨╛╨║╨╕. ╨Ч╨╜╨░╤З╨╡╨╜╨╕╤П ╨Р╨ж╨Я,
    ╨▓╨║╨╗╤О╤З╨░╤П ╨╜╨░╤Б╤Л╤Й╨╡╨╜╨╕╨╡ ╨╕ ╨┐╨╛╤Б╤В╨╛╤П╨╜╨╜╨╛╨╡ ╤Б╨╝╨╡╤Й╨╡╨╜╨╕╨╡, ╤П╨▓╨╗╤П╤О╤В╤Б╤П ╨┤╨╛╨┐╤Г╤Б╤В╨╕╨╝╤Л╨╝╨╕ ╨┤╨░╨╜╨╜╤Л╨╝╨╕ ╨╕
    ╨╜╨╡ ╨┤╨╛╨╗╨╢╨╜╤Л ╤Б╤А╨░╨▓╨╜╨╕╨▓╨░╤В╤М╤Б╤П ╤Б ╨░╨╝╨┐╨╗╨╕╤В╤Г╨┤╨╜╤Л╨╝ ╨┐╤А╨╛╤Д╨╕╨╗╨╡╨╝ ╨║╨╛╨╜╨║╤А╨╡╤В╨╜╨╛╨│╨╛ ╤Б╤В╨╡╨╜╨┤╨░. }
