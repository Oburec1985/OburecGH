unit uRecorderMic140Timing;

{ ╨в╨░╨╣╨╝╨╡╤А MC114 16 ╨Ь╨У╤Ж, SPORT-╨╖╨░╨┤╨╡╤А╨╢╨║╨╕ ╨┤╨╗╤П ╨┤╨╡╤Б╨║╤А╨╕╨┐╤В╨╛╤А╨╛╨▓. }
...
const
  { [ORIG] mic140_96mod.cpp / Mc114mod.cpp тАФ ╤Б╨╡╤В╨║╨░ 16 ╨Ь╨У╤Ж }
  CMic140FrequencyCount = 8;
  CMic140LegacyMaxUiReadFrequencyHz = 1000.0;  { LNX: ╨┐╨╛╤В╨╛╨╗╨╛╨║ UI, ╨╜╨╡ ╨▓ scale_period }
...
const
  { [ORIG] Mc114mod.cpp scale_period_16000 тАФ ╤З╨░╤Б╤В╨╛╤В╤Л UI MIC-140 }
...
begin
  ...
  { [ORIG] ModuleMC114::GetCountChansFor(flag_allch_sampl, flag_chan_ground):
    all channels on тЖТ GetMaxChanCount() = AIn+TIn; doubled only when ground on. }
...
begin
  ...
  Result.AverageSampleCount := Mic140v2CheckCountAver(lCountChans,
    ...
    CMic140LegacyMaxCountAver, AGroundEnabled);
