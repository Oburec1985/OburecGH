...

{
  ...
  Слои: Protocol / Scan / Stream / RawRing. Utils в MIC140/utils/.
}
...
begin
  SetLength(AValues, 0);
  AErrorText := 'Действие не поддерживается текущей реализацией API MIC-140';
  Result := False;
end;
...
begin
  ...
  { Если BIOS-буфер меньше half-FIFO (~7 отсчётов/канал для 48ch),
    то при 100 Гц и DataUpdateMs=300 надо 30, но FIFO даст 6-7. Config
    должен оставаться в рабочем диапазоне; недостающее дочитает DoTick. }
  lStride := ScanStride;
  if lStride <= 0 then
  ...
  { FIFO MIC-140 содержит в одной строке и измерительные, и температурные
    каналы. Кольцо обязано хранить строку целиком, иначе TIn теряет связь со
    временем и номером исходного сетевого пакета. }
  fDataThread.Config(lStride, lSamples, fFreq);
end;
...
begin
  ...
  { У MIC-140-48v3/SubRev1 видимые t6..t12 лежат подряд в DM 5..11.
    Читаем их одним пакетным запросом: это быстрее одиночных чтений и не
    разрывает температурный снимок между каналами внутри 200-ms цикла. }
  if lSubRev = 1 then
  begin
    lUseCache := False;
    if fCli.ReadDmWords(
  ...
  if lNewValues then
    Inc(fAuxRevision);
  fAux.Revision := fAuxRevision;
  fAux.ChannelCount := fTinSlots;
  { TIn публикуется отдельным температурным блоком DM, но его семпл привязан
    к последнему опубликованному FIFO AIn. }
  fAux.SampleCount := 1;
...
begin
  ...
  if (fState = rdsDisconnected) or (fCli = nil) or fInitialized then
    Exit;
  if not fCli.TestLoad(err) then
  begin
    Mic140v2Log(Format('[MIC140v2:%s:%d] init TEST_LOAD: %s',
      [fHost, fPort, err]));
    Exit;
  end;
  Mic140v2Log(Format('[MIC140v2:%s:%d] init TEST_LOAD: ready',
    [fHost, fPort]));
  ...
  { [ORIG: CCMC031EthernetInterface::LoadBios] программирование MIC-140-48v3
    всегда идёт через аппаратный CMD_RESET. Это совпадает с веткой
    ModuleMIC140_96::LoadBios: без сброса BIOS не стартует стабильно. }
  ...
  fCli.ClearBufferedPackets;
  { Reset переводит контроллер в промежуточное состояние. В оригинальном
    коде дальше выдерживается пауза для выхода CheckState(BIOS_LOADED_STATE).
    Без задержки Stop/ResetScanMain может прийти, пока BIOS ещё не готов. }
  { [ORIG] После CMD_RESET первый RESETSCANMAIN выполняется почти сразу,
    второй — после загрузки BIOS примерно через пять секунд. Config не должен
    начинаться раньше: на холодном контроллере прежний код успевал получить
    один FIFO-блок, после чего scan останавливался. }
  Sleep(CMic140LegacyBiosResetFirstDelayMs);
  if not fCli.ReadFirmware(fFw, err) then
  begin
  ...
  fHwSer := Mic140v2HardwareCalibrSerial(fFw);
  if not fCli.CallCommand(CMic140LegacyCmdResetScanMain, nil, 0, lReply,
    lResetScanErr) then
  begin
    Mic140v2Log(Format('[MIC140v2:%s:%d] init first RESETSCANMAIN: %s',
      [fHost, fPort, lResetScanErr]));
    Exit;
  end;
  Sleep(CMic140LegacyBiosResetSettleMs);
  if not fCli.CallCommand(CMic140LegacyCmdResetScanMain, nil, 0, lReply,
    lResetScanErr) then
  begin
    Mic140v2Log(Format('[MIC140v2:%s:%d] init second RESETSCANMAIN: %s',
      [fHost, fPort, lResetScanErr]));
    Exit;
  end;
...
begin
  ...
  if not fInitialized then
    Exit;
  if fConfiguredInSession then
  begin
    if not fCli.CallCommand(CMic140LegacyCmdResetScanMain, nil, 0, lReply,
      lError) then
      raise ERecorderDeviceError.Create(
        'MIC-140 reconfigure RESETSCANMAIN: ' + lError);
    if not Mic140v2StopScan(fCli, lError) then
      raise ERecorderDeviceError.Create(
        'MIC-140 reconfigure STOPSCANMAIN: ' + lError);
    fCli.ClearBufferedPackets;
  end;
  ProgramDevice;
  fConfiguredInSession := fState = rdsProgrammed;
end;
...
begin
  { Start не повторяет Connect/Init/Config заново: подготовленная сессия
    остаётся запрограммированной и только переводится в рабочее состояние. }
  if fState <> rdsProgrammed then
    Exit;
  if fCli = nil then
    Exit;

  { [ORIG] После Stop объект Device остаётся PROGRAMMED, поэтому повторный Start
    делает только Programming через SETSTATESCAN(scan_id, 0). В RecorderLnx
    Config выполняется до Play; если каждый старт программировать с нуля, прибор
    иногда зависает на подготовке, а повторный быстрый Start становится
    нестабильным. }
  { Автономный рабочий протокол перед STARTSCANMAIN явно переводит
    единственную циклограмму scan_id=0 в рабочее состояние. }
  SetLength(lArgs, 2);
...
begin
  { Play: чтение из DataThread; consumer (DataSource) берёт готовые блоки. }
...
begin
  { Как MIC-185: не тянем данные во время scan-потока и DataThread. }
  Result := False;
  AErrorText := '';
