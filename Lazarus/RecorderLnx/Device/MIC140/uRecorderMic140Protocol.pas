unit uRecorderMic140Protocol;

{ TCP + MDP, CC Ethernet BIOS. ╨в╨╛╨╗╤М╨║╨╛ v2. }

{
  ...
  2026-06: Mic140v2DeviceSerialFromFirmware ╨а╨Ж╨бтА╣╨а╨Е╨а┬╡╨б╨Г╨а┬╡╨а╨Е ╨б╨Г╨б╨Л╨а╥С╨а┬░ ╨а╤С╨а┬╖
  uRecorderMic140DataSource (╨б╨В╨а┬░╨а┬╖╨а┬▒╨а╤Х╨б╨В TBiosInfoMC031 / ETH81 identify).
}
  ...
  try
    ...
    try
      ...
      repeat
        ...
        { [ORIG] mdpEthernet81: stream 0 тЖТ scan FIFO, stream 1 тЖТ command reply. }
        if lPort = CLegacyStreamScan then
        begin
          AbsorbScanWords(lWords);
          Continue;
...
begin
  ...
  try
    try
      ...
      { ReadLegacyRawBlock(0) тЖТ 1 ms timeout: drain TCP into scan queue. }
...
begin
  Result := 0;
  { DevSerNo ╨╜╨░ Ethernet MIC ╤З╨░╤Б╤В╨╛ ╤Б╨╛╨▓╨┐╨░╨┤╨░╨╡╤В ╤Б ╨┐╨╛╤Б╨╗╨╡╨┤╨╜╨╕╨╝ ╨╛╨║╤В╨╡╤В╨╛╨╝ IP, ╨╜╨╡ ╤Б ╤Б/╨╜ MIC. }
...
begin
  ...
  { CCMC031EthernetInterface::ReadCfg: owner->DeviceInfo.SerialNo = CCSerNo;
    ...
    DevSerNo тАФ ╤В╨╛╨╗╤М╨║╨╛ ╨╕╨┤╨╡╨╜╤В╨╕╤Д╨╕╨║╨░╤В╨╛╤А Ethernet MDP, ╨╜╨╡ ╨║╨░╤В╨░╨╗╨╛╨│ calibr/hardware. }
