...
{
  ╠юфєы№ uRecorderSettingsDialog

  ═рчэрўхэшх:
    ─шрыюу эрёЄЁющъш ярЁрьхЄЁют ЁхъюЁфхЁр ш ъюэЇшуєЁрЎшш ряярЁрЄэ√ї ърэрыют/єёЄЁющёЄт.
    ╧ючтюы хЄ чрфртрЄ№ ярЁрьхЄЁ√ юЄюсЁрцхэш , сєЇхЁшчрЎшш, чряшёш, єёыютш 
    ёЄрЁЄр/юёЄрэютр ёсюЁр фрээ√ї, р Єръцх шьяюЁЄшЁютрЄ№ ёшуэры√ шч Їрщыют ЇюЁьрЄр Mera.

  ┴шсышюЄхъш ш чртшёшьюёЄш:
    - Classes, SysUtils, Forms, Controls, Graphics, Dialogs, StdCtrls, ExtCtrls: ёЄрэфрЁЄэ√х ьюфєыш LCL.
    - ComCtrls, ImgList, Grids, Buttons: ъюьяюэхэЄ√ UI (фхЁхтю єёЄЁющёЄт, ёяшёъш ърэрыют).
    - uRecorderStateMachine, uRecorderRunControlSettings, uRecorderTags: сшчэхё-ыюушър ЁхъюЁфхЁр.
    - uMeraFile: ярЁёшэу Їрщыют ъюэЇшуєЁрЎшш ёшуэрыют Mera.
    - uRecorderCommandImages: ъюэёЄрэЄ√ шэфхъёют шъюэюъ UI.
}
...
type
  ...
  { ╩ырёё фшрыюуютюую юъэр эрёЄЁюхъ ЁхъюЁфхЁр }
  TRecorderSettingsDialog = class(TForm)
  published
    fPageControl: TPageControl;                 // ╩юэЄхщэхЁ тъырфюъ эрёЄЁюхъ
    fApplyButton: TButton;                     // ╩эюяър "╧ЁшьхэшЄ№"
    fHardwareTree: TTreeView;                   // ─хЁхтю ряярЁрЄэющ ъюэЇшуєЁрЎшш/єёЄЁющёЄт
    btnDeviceAdd: TBitBtn;                      // ╩эюяър фюсртыхэш  єёЄЁющёЄтр (Mera-Їрщыр)
    btnChannelAdd: TBitBtn;                     // ╩эюяър фюсртыхэш  т√сЁрээюую ърэрыр т ёяшёюъ ръЄштэ√ї
    btnChannelRemove: TBitBtn;                  // ╩эюяър єфрыхэш  ърэрыр шч ёяшёър ръЄштэ√ї
    btnChannelEdit: TBitBtn;                    // ╩эюяър эрёЄЁющъш т√сЁрээюую Єхур
    pnChannelMoveButtons: TPanel;               // ╧рэхы№ ъэюяюъ яхЁхьх∙хэш  ърэрыют
    spChannels: TSplitter;                      // ╨рчфхышЄхы№ ьхцфє ёхЄърьш фюёЄєяэ√ї ш т√сЁрээ√ї ърэрыют
    fAvailableChannelsGrid: TStringGrid;        // ╥рсышЎр фюёЄєяэ√ї фы  т√сюЁр ърэрыют
    fSelectedChannelsGrid: TStringGrid;         // ╥рсышЎр т√сЁрээ√ї (ръЄштэ√ї) ърэрыют
    spChannelAlgorithms: TSplitter;             // ╨рчфхышЄхы№ ьхцфє ърэрырьш ш рыуюЁшЄьрьш
    fAlgorithmsTree: TTreeView;                 // ─хЁхтю рыуюЁшЄьют ърэрыют
    fAlgorithmKindCombo: TComboBox;             // ╥шя ёючфртрхьюую рыуюЁшЄьр
    btnAlgorithmAdd: TBitBtn;                   // ╤ючфрЄ№ рыуюЁшЄь яю т√сЁрээ√ь ърэрырь
    btnAlgorithmRemove: TBitBtn;                // ╙фрышЄ№ єчхы рыуюЁшЄьр
    btnAlgorithmConfig: TBitBtn;                // ╧ЁшьхэшЄ№ ярЁрьхЄЁ√ FFT-єчыр
    btnFrequencyBands: TBitBtn;                 // ═рёЄЁюшЄ№ ўрёЄюЄэ√х яюыюё√
    fAlgorithmFftSizeEdit: TEdit;               // ╨рчьхЁ FFT
    fAlgorithmFftSizeUpDown: TUpDown;           // ╤ЄЁхыюўъш шчьхэхэш  ЁрчьхЁр FFT
    fAlgorithmSampleRateEdit: TEdit;            // ╫рёЄюЄр юяЁюёр
    fAlgorithmPortionLabel: TLabel;             // ╨рчьхЁ яюЁЎшш т ёхъєэфрї
    fAlgorithmAverageBlocksEdit: TEdit;         // ╩юышўхёЄтю сыюъют єёЁхфэхэш 
    fAlgorithmOverlapEdit: TEdit;               // ╧хЁхъЁ√Єшх FFT
    fAlgorithmOverlapCombo: TComboBox;          // ╨хцшь яхЁхъЁ√Єш  FFT
    fAlgorithmWindowCombo: TComboBox;           // ╬ъюээр  ЇєэъЎш 
    fAlgorithmNormalizeCombo: TComboBox;        // ═юЁьшЁютър ёяхъЄЁр
    fAlgorithmZeroPadCheck: TCheckBox;          // ─юяюыэ Є№ эєы ьш
    fAlgorithmAhCorrectionCheck: TCheckBox;     // ╩юЁЁхъЎш  └╫╒
    fAlgorithmIntegrationGroup: TRadioGroup;    // ╨хцшь шэЄхуЁшЁютрэш 
    Cfg: TEdit;

    // ╧юы  ттюфр юс∙шї эрёЄЁюхъ
    fScreenUpdateEdit: TEdit;                   // ╧хЁшюф юсэютыхэш  ¤ъЁрэр (ёхъ)
    fBufferSecondsEdit: TEdit;                  // ─ышэр юЄюсЁрцрхьюую сєЇхЁр (ёхъ)
    fDataUpdateEdit: TEdit;                     // ╧хЁшюф юсэютыхэш  фрээ√ї (ёхъ)
    fTestNameEdit: TEdit;                       // ╚ь  Єхъє∙хую шёя√Єрэш 
    fProductNameEdit: TEdit;                    // ╚ь  шёёыхфєхьюую шчфхыш 
    fModifyNameCheck: TCheckBox;                // ╘ыру ртЄюьрЄшўхёъющ ьюфшЇшърЎшш шьхэш шёя√Єрэш 
    fPrehistoryCheck: TCheckBox;                // ╘ыру чряшёш яЁхф√ёЄюЁшш
    fPrehistoryEdit: TEdit;                     // ─ышэр яЁхф√ёЄюЁшш (ёхъ)
    fResetTimeCheck: TCheckBox;                 // ╘ыру ёсЁюёр тЁхьхэш яЁш ёЄрЁЄх чряшёш
    fWriteWithPausesCheck: TCheckBox;           // ╘ыру ЁрчЁх°хэш  чряшёш ё ярєчрьш
    fSaveConfigWithDataCheck: TCheckBox;        // ╘ыру ёюїЁрэхэш  Їрщыр ъюэЇшуєЁрЎшш тьхёЄх ё фрээ√ьш
    fWorkDirEdit: TEdit;                        // ╨рсюўшщ ърЄрыюу ёюїЁрэхэш  Їрщыют
    fTemplateCheck: TCheckBox;                  // ╘ыру шёяюы№чютрэш  °рсыюэр шьхэш Їрщыр
    fTemplateButton: TButton;                   // ╩эюяър эрёЄЁющъш °рсыюэр
    fFrameDirEdit: TEdit;                       // ╧єЄ№ ъ Єхъє∙хьє ърфЁє фрээ√ї

    // ╙ёыютш  ёЄрЁЄр чряшёш
    fStartManualRadio: TRadioButton;            // ╤ЄрЁЄ тЁєўэє■ (яю ъэюяъх)
    fStartLevelRadio: TRadioButton;             // ╤ЄрЁЄ яю фюёЄшцхэш■ єЁютэ  ёшуэрыр
    fStartTriggerRadio: TRadioButton;           // ╤ЄрЁЄ яю тэх°эхьє ЄЁшуухЁє
    fStartTriggerEdit: TEdit;                   // ═юьхЁ ЄЁшуухЁр ёЄрЁЄр
    fStartChannelCombo: TComboBox;              // ╩рэры-шёЄюўэшъ фы  єёыютш  ёЄрЁЄр
    fStartEdgeCombo: TComboBox;                 // ═ряЁртыхэшх яхЁхїюфр (сюы№°х/ьхэ№°х)
    fStartLevelEdit: TEdit;                     // ╧юЁюуют√щ єЁютхэ№ фы  ёЄрЁЄр

    // ╙ёыютш  юёЄрэютр чряшёш
    fStopManualRadio: TRadioButton;             // ╬ёЄрэют√ тЁєўэє■ (яю ъэюяъх)
    fStopLevelRadio: TRadioButton;              // ╬ёЄрэют яю єЁютэ■ ёшуэрыр
    fStopDurationRadio: TRadioButton;           // ╬ёЄрэют яю фышЄхы№эюёЄш (ЄрщьхЁє)
    fStopDurationEdit: TEdit;                   // ┬Ёхь  чряшёш фю юёЄрэютр (ёхъ)
    fStopChannelCombo: TComboBox;               // ╩рэры-шёЄюўэшъ фы  єёыютш  юёЄрэютр
    fStopEdgeCombo: TComboBox;                  // ═ряЁртыхэшх яхЁхїюфр фы  юёЄрэютр
    fStopLevelEdit: TEdit;                      // ╧юЁюуют√щ єЁютхэ№ фы  юёЄрэютр
    fStopReturnToPreviewCheck: TCheckBox;       // ╘ыру тючтЁрЄр т Ёхцшь яЁюёьюЄЁр яюёых юёЄрэютр

    // ╬сЁрсюЄўшъш ёюс√Єшщ UI ¤ыхьхэЄют фшрыюур
  ...
  private
    fRunSettings: TRecorderRunControlSettings;   // ╤ё√ыър эр юс·хъЄ эрёЄЁюхъ чряєёър/юёЄрэютр
    fTagRegistry: TRecorderTagRegistry;         // ╤ё√ыър эр ЁххёЄЁ Єхуют яЁшыюцхэш 
    fDeviceImageList: TCustomImageList;         // ╤яшёюъ ърЁЄшэюъ фы  фхЁхтр єёЄЁющёЄт
    fTagDialogImageList: TCustomImageList;      // ╤яшёюъ шъюэюъ фшрыюур эрёЄЁющъш Єхуют
    fMeraFolder: string;                        // ╧єЄ№ ъ яюёыхфэхщ яряъх шьяюЁЄшЁютрээюую Mera-Їрщыр
    fMeraFileName: string;                      // ╚ь  шьяюЁЄшЁютрээюую Mera-Їрщыр
    fMeraSignals: TList;                        // ╤яшёюъ TMeraSignalInfo, чруЁєцхээ√ї шч Їрщыр
    fMic140Signals: TList;                      // ╤яшёюъ фюёЄєяэ√ї ърэрыют MIC-140
    fSelectedChannelTags: TList;                // Row-map т√сЁрээ√ї ърэрыют эр TRecorderTag
    fSelectedSortColumn: Integer;               // ╩юыюэър Єхъє∙хщ ёюЁЄшЁютъш т√сЁрээ√ї ърэрыют
    fSelectedSortAscending: Boolean;            // ═ряЁртыхэшх Єхъє∙хщ ёюЁЄшЁютъш
    fSpectrumConfigTree: TRecorderSpectrumConfigTree; // ╫хЁэютр  ьюфхы№ рыуюЁшЄьют тъырфъш ърэрыют
    fFrequencyBands: TRecorderFrequencyBandList; // ╫хЁэютр  ьюфхы№ ўрёЄюЄэ√ї яюыюё
    fCanDrag: Boolean;
    fDragStartPt: TPoint;
    ...
    // ┬ёяюьюурЄхы№э√х ьхЄюф√ ЁрсюЄ√ ё Mera-ёшуэрырьш
    ...
    // ╠хЄюф√ шэшЎшрышчрЎшш ш юсэютыхэш  шэЄхЁЇхщёр
    ...
    // ─шэрьшўхёъюх яюёЄЁюхэшх UI (шёяюы№чєхЄё  яЁш юЄёєЄёЄтшш lfm-Їрщыр ЇюЁь√)
    procedure BuildUi;
  ...
  public
    ...
    // ╤тющёЄтр фюёЄєяр ъ чртшёшьюёЄ ь
...
{ ╬ЄюсЁрцрхЄ ьюфры№э√щ фшрыюу эрёЄЁюхъ }
...
{ ╥юўър тїюфр фы  чряєёър фшрыюур эрёЄЁюхъ }
function ShowRecorderSettingsDialog(AOwner: TComponent;
  ARunSettings: TRecorderRunControlSettings;
...
{ ┬ёяюьюурЄхы№э√х ЇєэъЎшш фшэрьшўхёъюую ёючфрэш  UI ъюэЄЁюыют }
...
{ ═рчэрўхэшх ърЁЄшэъш ъэюяърь ё уышЇюь }
...
begin
  ...
  begin
    ...
    btnChannelEdit.Hint := CP1251ToUTF8('═рёЄЁюшЄ№ т√сЁрээ√щ ърэры');
...
{ ═рёЄЁющър шъюэюъ ъэюяюъ эр ярэхыш фхЁхтр єёЄЁющёЄт }
...
begin
  Result := fSpectrumConfigTree.AddNode('spectrum.' +
    ...
    CP1251ToUTF8('╤яхъЄЁ ') + IntToStr(fSpectrumConfigTree.NodeCount + 1));
...
begin
  ...
  if fAlgorithmKindCombo.Text <> CP1251ToUTF8('╤яхъЄЁ') then
    Exit;
...
begin
  ...
  begin
    ...
    fAlgorithmKindCombo.Items.Add(CP1251ToUTF8('╤яхъЄЁ'));
    fAlgorithmKindCombo.ItemIndex := 0;
  end;
  if fAlgorithmWindowCombo <> nil then
  begin
...
begin
  ...
  try
    ...
    lRoot := fAlgorithmsTree.Items.Add(nil, CP1251ToUTF8('└ыуюЁшЄь√'));
    lRoot.Data := nil;
...
begin
  ...
  begin
    ...
    fAlgorithmPortionLabel.Caption := CP1251ToUTF8('яюЁЎш : ') +
      FormatFloat('0.###', lPortionSec) + CP1251ToUTF8(' ё');
  end
  else
    fAlgorithmPortionLabel.Caption := CP1251ToUTF8('яюЁЎш : - ё');
...
{ ╤ючфрэшх шыш юсэютыхэшх ърэрыют/Єхуют т ЁххёЄЁх эр юёэютх т√сЁрээ√ї ёшуэрыют Mera-Їрщыр }
...
var
  ...
  begin
    ...
    if (fMeraSignals <> nil) and SameText(ATag.SourceId, MeraSourceId(fMeraFileName)) then
    begin
      for K := 0 to fMeraSignals.Count - 1 do
      begin
        lSig := TMeraSignalInfo(fMeraSignals[K]);
    ...
    if (fMic140Signals <> nil) and (Pos('MIC-140:', ATag.SourceId) = 1) then
    begin
      for K := 0 to fMic140Signals.Count - 1 do
      begin
        lSig := TMeraSignalInfo(fMic140Signals[K]);
...
begin
  ...
  for I := fTagRegistry.TagCount - 1 downto 0 do
  begin
    lTag := fTagRegistry.Tags[I];
    if SameText(lTag.SourceId, MeraSourceId(fMeraFileName)) or (Pos('MIC-140:', lTag.SourceId) = 1) then
    begin
  ...
  for I := fTagRegistry.ActiveSourceCount - 1 downto 0 do
  begin
    lSourceId := fTagRegistry.ActiveSourceIds[I];
    lCount := 0;
    for J := 0 to fTagRegistry.TagCount - 1 do
      if SameText(fTagRegistry.Tags[J].SourceId, lSourceId) then
        Inc(lCount);
    if lCount = 0 then
...
{ ┬ючтЁр∙рхЄ Mera-ёшуэры яю ёЄЁюъх ЄрсышЎ√ фюёЄєяэ√ї ърэрыют }
...
{ ┬ючтЁр∙рхЄ Mera-ёшуэры яю ёЄЁюъх ЄрсышЎ√ т√сЁрээ√ї ърэрыют }
...
{ ╟руЁєчър шэЇюЁьрЎшш ю ёшуэрырї шч т√сЁрээюую Їрщыр Mera }
...
{ ╬сэютыхэшх фхЁхтр ряярЁрЄэющ ўрёЄш яЁш чруЁєчъх Їрщыют Mera }
...
begin
  ...
  if MessageDlg(CP1251ToUTF8('╙фрыхэшх шёЄюўэшър фрээ√ї'),
    CP1251ToUTF8('╙фрышЄ№ шёЄюўэшъ фрээ√ї "') + ExtractFileName(fMeraFileName) + '"?',
...
begin
  ...
  begin
    MessageDlg(CP1251ToUTF8('╧хЁхўшЄ√трэшх шёЄюўэшър'),
      CP1251ToUTF8('╘рщы шёЄюўэшър фрээ√ї эх эрщфхэ: ') + fMeraFileName,
      mtWarning, [mbOK], 0);
    Exit;
  end;
...
begin
  ...
  try
    lDialog.Title := CP1251ToUTF8('┬√схЁшЄх Їрщы Mera');
...
begin
  ...
  try
    ...
    lRootNode := fHardwareTree.Items.Add(nil, CP1251ToUTF8('╙ёЄЁющёЄтр'));
...
{ ╙ёЄрэютър чруюыютъют ёхЄюъ ърэрыют }
...
begin
  ...
  begin
    ...
    fAvailableChannelsGrid.Cells[0, 0] := CP1251ToUTF8('└фЁхё');
    fAvailableChannelsGrid.Cells[1, 0] := CP1251ToUTF8('╥шя');
    fAvailableChannelsGrid.Cells[2, 0] := CP1251ToUTF8('╚ь ');
  ...
  begin
    ...
    fSelectedChannelsGrid.Cells[0, 0] := CP1251ToUTF8('╚ь ');
    fSelectedChannelsGrid.Cells[1, 0] := CP1251ToUTF8('└фЁхё');
    fSelectedChannelsGrid.Cells[2, 0] := CP1251ToUTF8('╥шя');
    fSelectedChannelsGrid.Cells[3, 0] := CP1251ToUTF8('╫рёЄюЄр');
    fSelectedChannelsGrid.Cells[4, 0] := CP1251ToUTF8('├╒');
    fSelectedChannelsGrid.Cells[5, 0] := CP1251ToUTF8('├Ёєяяр');
    fSelectedChannelsGrid.Cells[6, 0] := CP1251ToUTF8('╚эЇюЁьрЎш ');
...
{ ╟ряюыэхэшх ёхЄюъ фюёЄєяэ√ї ш т√сЁрээ√ї ърэрыют эр юёэютх fMeraSignals }
...
{ ╧хЁхъы■ўхэшх ръЄштэюёЄш ёшуэрыр яю фтющэюьє ъышъє т фхЁхтх ряярЁрЄэ√ї ьюфєыхщ }
...
begin
  ...
  begin
    ...
    lItem.Caption := CP1251ToUTF8('╧хЁхўшЄрЄ№ Єхуш шёЄюўэшър');
    lItem.OnClick := @HardwareReloadSourceClick;
    lPopup.Items.Add(lItem);

    lItem := TMenuItem.Create(lPopup);
    lItem.Caption := CP1251ToUTF8('═рёЄЁющър шёЄюўэшър...');
    lItem.OnClick := @HardwareEditSourceClick;
    lPopup.Items.Add(lItem);

    lItem := TMenuItem.Create(lPopup);
    lItem.Caption := CP1251ToUTF8('╙фрышЄ№ шёЄюўэшъ фрээ√ї');
...
{ ─шэрьшўхёъюх ёючфрэшх яюы№чютрЄхы№ёъюую шэЄхЁЇхщёр }
...
begin
  ...
  lTab.Caption := CP1251ToUTF8('╨хъюЁфхЁ');
  BuildRecorderTab(lTab);

  lTab := TTabSheet.Create(Self);
  lTab.PageControl := fPageControl;
  lTab.Caption := CP1251ToUTF8('└яярЁрЄэ√х ётющёЄтр');
  BuildHardwareTab(lTab);

  BuildPlaceholderTab(CP1251ToUTF8('╩рэры√'));
  BuildPlaceholderTab(CP1251ToUTF8('╧ырушэ√'));
  ...
  lButton.AnchorSideRight.Side := asrRight;
  lButton.Anchors := [akTop, akRight];
  lButton.Caption := CP1251ToUTF8('╟ръЁ√Є№');
  lButton.Cancel := True;
  lButton.ModalResult := mrCancel;
  ...
  fApplyButton.Caption := CP1251ToUTF8('╧ЁшьхэшЄ№');
  fApplyButton.OnClick := @ApplyButtonClick;
end;

{ ╩юэёЄЁєшЁютрэшх тъырфъш юс∙шї эрёЄЁюхъ }
...
begin
  ...
  lGroup := AddGroup(Self, lLeftPanel, 8, 8, 210, 82, CP1251ToUTF8('╬ЄюсЁрцхэшх'));
  AddLabel(Self, lGroup, 10, 22, CP1251ToUTF8('╧хЁшюф юсэютыхэш '));
  fScreenUpdateEdit := AddEdit(Self, lGroup, 126, 18, 56, '0.5');
  AddLabel(Self, lGroup, 186, 22, CP1251ToUTF8('ё'));

  lGroup := AddGroup(Self, lLeftPanel, 238, 8, 390, 82, CP1251ToUTF8('╤шуэры√'));
  AddLabel(Self, lGroup, 10, 18, CP1251ToUTF8('─ышэр юЄюсЁрцрхь√ї фрээ√ї'));
  fBufferSecondsEdit := AddEdit(Self, lGroup, 190, 14, 64, '1');
  AddLabel(Self, lGroup, 260, 18, CP1251ToUTF8('ё'));
  AddLabel(Self, lGroup, 10, 40, CP1251ToUTF8('╧хЁшюф юсэютыхэш  фрээ√ї'));
  fDataUpdateEdit := AddEdit(Self, lGroup, 190, 36, 64, '0.3');
  AddLabel(Self, lGroup, 260, 40, CP1251ToUTF8('ё'));

  lGroup := AddGroup(Self, lLeftPanel, 8, 102, 620, 78, '');
  AddLabel(Self, lGroup, 10, 18, CP1251ToUTF8('╚ёя√Єрэшх'));
  fTestNameEdit := AddEdit(Self, lGroup, 130, 14, 470, CP1251ToUTF8('╚ёя√Єрэшх'));
  AddLabel(Self, lGroup, 10, 42, CP1251ToUTF8('╚чфхышх'));
  fProductNameEdit := AddEdit(Self, lGroup, 130, 42, 470, CP1251ToUTF8('╚чфхышх'));

  lGroup := AddGroup(Self, lLeftPanel, 8, 190, 620, 284, CP1251ToUTF8('╟ряшё№'));
  fModifyNameCheck := AddCheck(Self, lGroup, 12, 22,
    CP1251ToUTF8('╠юфшЇшЎшЁютрЄ№ шь  яю ърцфюьє шёя√Єрэш■'));
  fModifyNameCheck.Enabled := False;
  fPrehistoryCheck := AddCheck(Self, lGroup, 12, 48, CP1251ToUTF8('╧Ёхф√ёЄюЁш '));
  fPrehistoryEdit := AddEdit(Self, lGroup, 130, 44, 68, '10');
  AddLabel(Self, lGroup, 206, 48, CP1251ToUTF8('ёхъ'));
  fResetTimeCheck := AddCheck(Self, lGroup, 12, 74,
    CP1251ToUTF8('╤сЁюё тЁхьхэш яЁш эрўрых чряшёш'));
  fWriteWithPausesCheck := AddCheck(Self, lGroup, 12, 100, CP1251ToUTF8('╟ряшё№ ё ярєчрьш'));
  fSaveConfigWithDataCheck := AddCheck(Self, lGroup, 12, 126,
    CP1251ToUTF8('╤юїЁрэ Є№ Їрщы ъюэЇшуєЁрЎшш тьхёЄх ё чряшё№■ фрээ√ї'));
  AddLabel(Self, lGroup, 10, 154, CP1251ToUTF8('╨рсюўшщ ърЄрыюу'));
  ...
  fTemplateCheck := AddCheck(Self, lGroup, 12, 204, CP1251ToUTF8('╪рсыюэ'));
  ...
  fTemplateButton.Caption := CP1251ToUTF8('═рёЄЁюшЄ№');
  fTemplateButton.Enabled := False;
  fFrameDirEdit := AddEdit(Self, lGroup, 10, 232, 470, 'C:\USML\signal0000');

  lGroup := AddGroup(Self, lRightPanel, 8, 8, 210, 142, CP1251ToUTF8('╙ёыютш  ёЄрЁЄр чряшёш'));
  fStartManualRadio := AddRadio(Self, lGroup, 10, 20, CP1251ToUTF8('╧ю ъыртш°х'), @ConditionChanged);
  fStartLevelRadio := AddRadio(Self, lGroup, 104, 20, CP1251ToUTF8('╧ю єЁютэ■'), @ConditionChanged);
  fStartTriggerRadio := AddRadio(Self, lGroup, 10, 46, CP1251ToUTF8('╥ЁшуухЁэ√щ ёЄрЁЄ'), @ConditionChanged);
  fStartTriggerEdit := AddEdit(Self, lGroup, 140, 42, 48, '1');
  AddLabel(Self, lGroup, 10, 76, CP1251ToUTF8('╩рэры'));
  fStartChannelCombo := AddCombo(Self, lGroup, 52, 72, 136);
  fStartEdgeCombo := AddCombo(Self, lGroup, 10, 100, 74);
  fStartEdgeCombo.Items.Add(CP1251ToUTF8('ьхэ№°х'));
  fStartEdgeCombo.Items.Add(CP1251ToUTF8('сюы№°х'));
  fStartLevelEdit := AddEdit(Self, lGroup, 92, 100, 72, '0.0');

  lGroup := AddGroup(Self, lRightPanel, 8, 160, 210, 170, CP1251ToUTF8('╙ёыютш  юёЄрэютр чряшёш'));
  fStopManualRadio := AddRadio(Self, lGroup, 10, 20, CP1251ToUTF8('╧ю ъыртш°х'), @ConditionChanged);
  fStopLevelRadio := AddRadio(Self, lGroup, 104, 20, CP1251ToUTF8('╧ю єЁютэ■'), @ConditionChanged);
  fStopDurationRadio := AddRadio(Self, lGroup, 10, 46, CP1251ToUTF8('╫хЁхч'), @ConditionChanged);
  fStopDurationEdit := AddEdit(Self, lGroup, 70, 42, 74, '1.000000');
  AddLabel(Self, lGroup, 152, 46, CP1251ToUTF8('ёхъ'));
  AddLabel(Self, lGroup, 10, 76, CP1251ToUTF8('╩рэры'));
  fStopChannelCombo := AddCombo(Self, lGroup, 52, 72, 136);
  fStopEdgeCombo := AddCombo(Self, lGroup, 10, 100, 74);
  fStopEdgeCombo.Items.Add(CP1251ToUTF8('ьхэ№°х'));
  fStopEdgeCombo.Items.Add(CP1251ToUTF8('сюы№°х'));
  fStopLevelEdit := AddEdit(Self, lGroup, 92, 100, 72, '0.0');
  fStopReturnToPreviewCheck := AddCheck(Self, lGroup, 10, 126, CP1251ToUTF8('╧хЁхїюф т яЁюёьюЄЁ'));
  ...
  lButton.Width := 122;
  lButton.Height := 28;
  lButton.Caption := CP1251ToUTF8('╤шёЄхьэюх тЁхь ');
...
{ ╩юэёЄЁєшЁютрэшх тъырфъш фхЁхтр юсюЁєфютрэш  ш єёЄЁющёЄт }
...
var
  ...
  lButton: TButton;
begin
  lGroup := AddGroup(Self, ATab, 8, 8, 858, 560, CP1251ToUTF8('╙ёЄЁющёЄтр'));
  ...
  lControllerNode := fHardwareTree.Items.AddChild(lRootNode,
    CP1251ToUTF8('[1] ╠╤-╩ЁхщЄ - ISA ╩ЁхщЄ-ъюэЄЁюыыхЁ s/n: 0000'));
  ...
  with fHardwareTree.Items.AddChild(lControllerNode,
    CP1251ToUTF8('╤ыюЄ 1 - MC-212 ё/э:00000 - ╥хэчюьюфєы№ 4 ърэрыр v4.0-v5.0')) do
  begin
    ImageIndex := CDeviceModuleImageIndex;
    SelectedIndex := CDeviceModuleImageIndex;
  end;
  ...
  lButton.Height := 36;
  lButton.Caption := '+';
  lButton.Hint := CP1251ToUTF8('─юсртшЄ№ єёЄЁющёЄтю тЁєўэє■');
  ...
  lButton.Height := 36;
  lButton.Caption := '-';
  lButton.Hint := CP1251ToUTF8('╙фрышЄ№ єёЄЁющёЄтю тЁєўэє■');
  lButton.ShowHint := True;
  ...
  lButton.Height := 36;
  lButton.Caption := '...';
  lButton.Hint := CP1251ToUTF8('═рёЄЁюшЄ№ т√сЁрээюх єёЄЁющёЄтю');
  lButton.ShowHint := True;
  ...
  lButton.Height := 36;
  lButton.Caption := '?';
  lButton.Hint := CP1251ToUTF8('└тЄюяюшёъ яюфъы■ўхээ√ї єёЄЁющёЄт');
  lButton.ShowHint := True;
end;

{ ┬ёяюьюурЄхы№эр  чруыє°ър тъырфъш фы  х∙╕ эх ЁрчЁрсюЄрээ√ї ъюьяюэхэЄют }
...
begin
  ...
  lLabel.Left := 16;
  lLabel.Top := 16;
  lLabel.Caption := CP1251ToUTF8('╨рчфхы сєфхЄ чряюыэхэ яюёых яю тыхэш  ёююЄтхЄёЄтє■∙хщ ьюфхыш.');
end;

{ ╫Єхэшх ъюэЇшуєЁрЎшш шч юс·хъЄр TRecorderRunControlSettings т UI ¤ыхьхэЄ√ фшрыюур }
...
{ ╧хЁхэюё эрёЄЁюхъ шч UI т юс·хъЄ fRunSettings }
...
{ ┬ъы■ўхэшх/т√ъы■ўхэшх ъюэЄЁюыыхЁют т чртшёшьюёЄш юЄ т√сЁрээ√ї ЄЁшуухЁют ёЄрЁЄр/юёЄрэютр }
...
{ ┴хчюярёэюх ўЄхэшх тх∙хёЄтхээюую ўшёыр шч TEdit ё єў╕Єюь ыюърыш }
...
{ ╫Єхэшх ёхъєэф шч UI ш яхЁхтюф т ьшыышёхъєэф√ }
...
begin
  ...
  if lDir = '' then
    lDir := 'C:\USML\';
  if not SelectDirectory(CP1251ToUTF8('┬√схЁшЄх Ёрсюўшщ ърЄрыюу фы  чряшёш MERA-Їрщыют'), '', lDir) then
    Exit;
...
{ ─юсртыхэшх эютюую єёЄЁющёЄтр яєЄ╕ь шьяюЁЄр ёшуэрыют шч Їрщыр Mera }
procedure TRecorderSettingsDialog.btnDeviceAddClick(Sender: TObject);
var
  lDialog: TOpenDialog;
begin
  if MessageDlg(CP1251ToUTF8('─юсртыхэшх єёЄЁющёЄтр'), 'Mera file', mtConfirmation,
...
{ ─юсртыхэшх ърэрыют т ръЄштэ√щ ёяшёюъ }
...
{ ╚ёъы■ўхэшх ърэрыют шч ръЄштэюую ёяшёър }
...
begin
  try
    ...
    on E: Exception do
      MessageDlg(CP1251ToUTF8('═рёЄЁющър ёяхъЄЁр'), E.Message, mtError, [mbOK], 0);
  end;
end;
...
begin
  ...
  if lProfile = '' then
    lProfile := 'default';
  if InputQuery(CP1251ToUTF8('╩юЁЁхъЎш  └╫╒'), CP1251ToUTF8('╧ЁюЇшы№ ъю¤ЇЇшЎшхэЄют ёяхъЄЁр'), lProfile) then
  begin
...
{ Drag and Drop фюёЄєяэюую ърэрыр т ръЄштэє■ ёхЄъє }
...
{ ─тющэющ ъышъ эр єёЄЁющёЄтх/ьюфєых т фхЁхтх фы  шчьхэхэш  хую ръЄштэюёЄш }
...
begin
  ...
  lSelected := fHardwareTree.Selected;
  if lSelected.Data = nil then
  begin
    if (lSelected.Parent <> nil) and (lSelected.Parent.Parent = nil) then
      HardwareEditSourceClick(Sender);
  end
  else
  begin
    ToggleHardwareSignal(lSelected);
    if fPageControl <> nil then
