...
begin
  Result := False;
  lSourceId := FindMic185SourceId(ARecorder.TagRegistry);
  if lSourceId = '' then
  begin
  ...
  try
    ...
    RecorderSettingsDialogDebugEditMic185(AOwner, ARecorder,
...
begin
  ...
  lHost.SourceId := FindMic185SourceId(ARecorder.TagRegistry);
  lHost.Phase := 0;
  lHost.WaitTicks := 0;
    ...
    1:
      begin
        ...
        begin
          ...
          lOk := ExecuteRecorderMic185SettingsSelfTest(Owner, Recorder,
            DeviceImages, TagImages, nil, DataSourcesRunning, Log);
          if Assigned(Log) then
          begin
            if lOk then
