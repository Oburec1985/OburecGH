...

{
  ...
  Purpose:
    Dynamic settings forms (no DFM) for mnemonic visual components.
    Includes TagName substring filtering / search, Font selection via TFontDialog,
    and component-specific configurations (StaticText, TagValue, Oscillogram).
}
...
uses
  ...
  Dialogs, uRecorderFormModel, uRecorderTags;

type
  { TComponentSettingsDialog }
  ...
  private
    ...
    // Font parameters (StaticText and TagValue)
...
begin
  ...
  // 1. Поиск и выбор тега (для TagValue и Oscillogram)
  ...
  // 2. Специфические настройки для StaticText
  ...
  // 3. Настройка шрифта (для StaticText и TagValue)
  ...
  // 4. Специфические настройки для TagValue
  ...
  begin
    ...
    fShowNameCombo.Items.Add('Имя сверху');
  ...
  // 5. Специфические настройки для Oscillogram
  ...
  begin
    ...
    fBindingModeCombo.SetBounds(140, lTop, 180, 23);
...
begin
  ...
  try
    ...
    begin
      ...
      if (lFilter = '') or (Pos(lFilter, LowerCase(lTag.Name)) > 0) then
        fTagCombo.Items.Add(lTag.Name);
    end;
    
    I := fTagCombo.Items.IndexOf(lCurrentSelection);
    if I >= 0 then
...
begin
  ...
  try
    ...
    lDlg.Font.Style := [];
    if fSelectedFontBold then lDlg.Font.Style := lDlg.Font.Style + [fsBold];
    if fSelectedFontItalic then lDlg.Font.Style := lDlg.Font.Style + [fsItalic];
...
begin
  if fFontPreviewLabel = nil then Exit;
  ...
  if fSelectedFontBold then fFontPreviewLabel.Font.Style := fFontPreviewLabel.Font.Style + [fsBold];
  if fSelectedFontItalic then fFontPreviewLabel.Font.Style := fFontPreviewLabel.Font.Style + [fsItalic];
end;
...
var
  lTagIndex: Integer;
begin
  // Инициализация выпадающего списка тегов
  if fTagCombo <> nil then
  begin
    PopulateTags('');
    lTagIndex := fTagCombo.Items.IndexOf(fComponent.TagName);
    if lTagIndex >= 0 then
...
begin
  ...
  begin
    // rtbmRelativeSelectedTag = 0, rtbmAbsoluteTag = 1
    // "Если выбрана относительная привязка, то не показывай элементы с выбором канала"
