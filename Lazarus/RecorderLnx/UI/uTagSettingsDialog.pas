...
begin
  ...
  try
    lForm.Caption := CP1251ToUTF8('Выбор сигнала источника');
    ...
    lCancelBtn.Caption := CP1251ToUTF8('Отмена');
...
begin
  ...
  try
    lDialog.Caption := CP1251ToUTF8('Выбор канала источника данных');
    ...
    lButton.Caption := CP1251ToUTF8('Отмена');
    lButton.Cancel := True;
    ...
    begin
      MessageDlg(CP1251ToUTF8('Выбор канала'), CP1251ToUTF8('Нет доступных каналов активного источника данных.'),
...
begin
  ...
  begin
    MessageDlg(CP1251ToUTF8('Выбор канала'),
      CP1251ToUTF8('Адрес можно связывать с источником только для одного выбранного тега.'),
      mtInformation, [mbOK], 0);
    Exit;
  end;
...
begin
  if fTags.Count = 1 then
  begin
    Caption := CP1251ToUTF8('Настройка канала ') + TagAt(0).Name;
  ...
  begin
    Caption := Format(CP1251ToUTF8('Настройка каналов (%d)'), [fTags.Count]);
    fNameEdit.Text := '';
...
begin
  if fTags.Count <> 1 then
  begin
    MessageDlg(CP1251ToUTF8('Канальная ГХ'),
      CP1251ToUTF8('Выбор ГХ из списка доступен только для одного выбранного тега.'),
      mtInformation, [mbOK], 0);
    Exit;
  end;
...
begin
  ...
  try
    lCalibration.Name := CP1251ToUTF8('ГХ ') + IntToStr(fTagRegistry.Calibrations.Count + 1);
...
begin
  ...
  begin
    MessageDlg(CP1251ToUTF8('Канальная ГХ'),
      CP1251ToUTF8('ГХ можно редактировать только для одного выбранного тега.'),
      mtInformation, [mbOK], 0);
    Exit;
  end;
  ...
  begin
    MessageDlg(CP1251ToUTF8('Канальная ГХ'),
      CP1251ToUTF8('У выбранного тега нет назначенной ГХ.'),
      mtInformation, [mbOK], 0);
    Exit;
  end;
  ...
  begin
    MessageDlg(CP1251ToUTF8('Канальная ГХ'),
      CP1251ToUTF8('Назначенная ГХ не найдена в списке характеристик.'),
      mtInformation, [mbOK], 0);
    Exit;
  end;
