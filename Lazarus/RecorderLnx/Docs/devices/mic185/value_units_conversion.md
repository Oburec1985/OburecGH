...
Source/tag unit ownership rule, 2026-08-25:

- Физическая единица измерительного канала (`мВ`, `Ом`, `мкм/м`,
  `мВ(тензо)`) принадлежит настройке источника MIC-185 и сохраняется в
  `channels[].unitName` независимо от режима тега.
- `HardwareCalibrationEnabled=False` у тега означает временный raw-режим:
  `Tag.UnitName=код`, runtime-transform строго `K=1, B=0`, аппаратная,
  единичная и канальные ГХ не применяются. Это не меняет `channels[].unitName`.
- При повторном включении аппаратной ГХ тег обязан восстановить единицу из
  `channels[].unitName`; `RecorderMic185RangeUnitText` допустим только как
  fallback для старого проекта, где настройка источника ещё не сохранена.
- Проверка: `Ом -> GX off -> OK -> reopen -> GX on -> OK` даёт сначала `код`,
  затем снова `Ом`; после save/reload результат сохраняется. Для группового
  переключения каждый тег восстанавливает единицу своего канала источника.
- Служебные физические значения в настройках источника, включая аппаратную и
  программную балансировку, отображать в выбранной `channels[].unitName`.
  Хранимый protocol/DAC/ADC code и его мВ-эквивалент не менять: для UI выполнять
  только unit-conversion `мВ -> мВ/Ом/мкм/м` с тем же калиброванным током и
  параметрами тензосхемы, что используются для диапазона канала.
