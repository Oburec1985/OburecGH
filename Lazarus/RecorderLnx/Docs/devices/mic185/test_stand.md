...
## Сетевой адрес рабочего интерфейса

Для интерфейса стенда предпочтительны DHCP и закрепление адреса на DHCP-сервере.
Перед назначением статического адреса обязательно проверить его уникальность
после временного снятия адреса с локальной карты: `ping <IP>` и `arp -a` не
должны показывать другой узел. Конфликт адреса проявляется не только ошибкой
подключения: соединение может сначала работать, а затем оборваться при
обновлении ARP-кэша; broadcast-поиск при этом также становится ненадёжным.
