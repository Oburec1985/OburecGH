...
`count_aver = 327` не определяет частоту; наоборот, при открытии диалога уже
существующая `freq` подаётся в `CheckPeriodDelay`, и из неё получается 327.
...
Практическое совпадение с диалогом Recorder для MIC-140-48 (`10 Гц`,
`period_decay=57/200 µs`, 51 слот, земля выключена) получается не при полном
значении `112`, а при эффективной поправке **103 тика**:

```text
K_eff = 1 + 103 / 640 = 1.1609375
```

Именно этот эффективный коэффициент используется в Codex-расчёте `count_aver`.
Частота канала при этом остаётся `10.000000 Гц`; `9.875 Гц` здесь не нужен.
...
### 8.9.1. Почему на скриншоте Recorder получается 327 / 298

В проверенном диалоге Recorder использует MIC-140-48, опрос всех каналов,
земля выключена, то есть `N_ch = 48 AIn + 3 TIn = 51`. Частота канала из
`GetFreqSFor` при выбранных 10 Гц остаётся **10.000000 Гц**. Совпадение с
Recorder достигается не изменением Fs, а эффективной поправкой времени
обработчика таймера: `K_eff = 1 + 103/640`.

| Fs, которую передали в `CheckPeriodDelay` | Поправка ISR | `period_decay = 57 µs` | `period_decay = 200 µs` |
|-------------------------------------------|--------------|------------------------|-------------------------|
| 10.000000 Гц                              | `112/640`    | 323                    | 294                     |
| **10.000000 Гц**                          | **`103/640`**| **327**                | **298**                 |

Разница +4 точки возникает потому, что эффективный «налог» ISR меньше:
`112/640 = 17.5%`, а `103/640 = 16.09375%`. После деления на 51 слот это даёт
примерно +20 µs полезного бюджета на слот, то есть ещё около четырёх периодов
усреднения по 5 µs.

При частотах 20/25/50/100 Гц штатная legacy-сетка остаётся ровной при
`GetFreqClk() = 16 MHz`; меняется только бюджет кадра через тот же `K_eff`.

Контрольные точки после последней сверки с Recorder:

| Fs канала (`GetFreqSFor`) | `period_decay` | `period_decay2` | GND | Слотов | `count_aver` Recorder/Codex |
|---------------------------|----------------|-----------------|-----|--------|-----------------------------|
| 10.000000 Гц              | 57.000 µs      | 19.688 µs       | off | 51     | 327                         |
| 10.000000 Гц              | 200.000 µs     | 19.688 µs       | off | 51     | 298                         |
| 100.000000 Гц             | 57.000 µs      | 19.688 µs       | off | 51     | 23                          |

Для 100 Гц и `period_decay = 200 µs` задержка уже не помещается в бюджет 51
слота: ветка `AUTO_CALC_COUNT_AVER` сначала зажимает `period_decay` через
`CheckPeriodDecay`, затем `CheckCountAver` опускает усреднение к допустимому
минимуму.
