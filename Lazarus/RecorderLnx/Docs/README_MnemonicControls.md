...
## Фоновое изображение пользовательской страницы

Пользовательская мнемосхема хранит путь к фону в
`TRecorderFormPage.BackgroundImageFileName`. Настройка доступна в диалоге
управления страницами: для выбранной mnemonic page можно выбрать PNG, JPEG или
BMP либо очистить фон. Встроенные страницы `DigitalForm` и `BasePage` этой
настройкой не изменяются.

В `<project>.gui.ini` поле `BackgroundImage` сохраняется относительно самого
INI-файла с прямыми разделителями каталогов. При загрузке путь преобразуется в
абсолютный путь текущей ОС. Поэтому изображение следует держать внутри каталога
проекта или переносить вместе с ним.

`TRecorderPagePanel` загружает `TPicture` только при смене пути и кэширует его.
Метод `Paint` выполняет только растягивание уже загруженной картинки на размер
страницы; файловый ввод-вывод и создание объектов в цикле обновления запрещены.
