// Общий порядок настройки и использования ТМ плат с помощью функций библиотеки mxxxx.dll
// 1.	Автоматический поиск ТМ модулей PCI с помощью функции SearchDevices.
// При необходимости работы с ISA модулями, нужно их добавить в структуру TDeviceEnum
// помощью функции AddISADevice. Шаг 1 необходимо выполнять один раз перед
// осуществлением остальных действий.
// 2.	Первичная инициализация ТМ модуля, с которым предполагается дальнейшая
// с помощью функции CreateDevice.
// 3.	Считываем настройки задачи из INI файла с помощью функции ReadPropertyEx.
// 4.	Устанавливаем адрес функции – обработчика прерывания с помощью функции
// setInterrupt.
// 5.	Конфигурируем ТМ модуль с помощью функции Config
// 6.	Загружаем в ТМ модуль его БИОС и ФЛЕКС с помощью функции Load.
// 8.	Подготавливаем буфер для приема/передачи данных. Размер буфера определяем
// помощью функции GetMaxBufSize.
// 7.	Инициализируем и запускаем сконфигурированную задачу с помощью функции Start.
// 8.	После получения данных и их обработки, выполняемых внутри обработчика прерывания,
// останавливаем работу задачи с помощью функции Stop.
// Шаги 3-8 могут выполняться в заданной последовательности несколько раз.
// 9.	Освобождаем ресурсы, занятые текущим модулем с помощью функции CloseDevice.
...
Type
  ...
  PIRQEvent=^TIRQEvent;
  TIRQEvent=Record
      scan_id       : LongWord;       // Id скана
      event_id      : LongWord;       // Id самого события
      time          : LongWord;       // время 
      local_number  : LongWord;       // ...доп. параметры...
      global_number : LongWord;       // ...доп. параметры...
  end;
  THandler=Function(Device:Pointer;Event:PIRQEvent;Data:Pointer):Integer;cdecl;


  // Работа с 2070. Заплняет список DeviceEnum найдеными в системе платами
  function SearchDevicesEx(DeviceEnum : PDeviceEnum) : cardinal;  stdcall;
  // выделяем память от устройства device
  function CreateDevice(DevInfo : TDevice; device : ppointer) : cardinal; stdcall;
  // освобождает память от устройства device
  function CloseDevice(device : ppointer)  : cardinal;  stdcall;
  // Загружает Bios и прошивку Flex в ТМ модуль (путь и имя файлов которых
  // в файле *.ini).
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0) в случае
  // или коды ошибки: ERROR_BIOS_CODE_NOT_FOUND=14, ERROR_FLEX_CODE_NOT_FOUND=15
  // случае если соответствующие файлы не найдены, либо ERROR_BIOS_NOT_LOADED=18,
  // RROR_FLEX_NOT_LOADED=19 в случае обнаружения ошибки загрузки, либо
  // ERROR_DEVICE_NOT_RESPOND=25 если устройство не ответило после загрузки Flex.
  ...
  // конфигурирует ТМ модуль.
  // В качестве параметра функции передается хендл модуля.
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0) в случае успеха
  // код ошибки ERROR_DMALOCK=35 если не удалось зарезервировать память
  ...
  // Функция производит настройку ТМ модуля параметрами, считанными из *.ini и
  // модуль на выполнение сконфигурированной задачи.
  // В качестве параметра функции передается хендл модуля.
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0) в случае
  // или код ошибки ERROR_DEVICE_NOT_RESPOND=25, если устройство не ответило.
  function Start(device : ppointer) : cardinal;  stdcall;
  // останавливает выполнение текущей задачи.
  // В качестве параметра функции передается хендл модуля.
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0) в случае успеха или код ошибки ERROR_DEVICE_NOT_RESPOND=25, если устройство не ответило.
  function Stop(device : ppointer) : cardinal;  stdcall;
  // Функция через параметр direction возвращает признак направления потока. Обращение к функции имеет смысл внутри обработчика прерывания.
  // В качестве первого параметра функции передается хендл модуля.
  // Direction принимает значение 0 если работает задача приема, 1- если работает задача передачи.
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0).
  ...
  // Функция считывает из *.ini файла, путь и имя которого указываются параметром
  // pFileName, настройки задачи для ТМ модуля, имя указываются параметром lpAppName.
  // В качестве параметра функции передается хендл модуля.
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0) в случае успеха
  // код ошибки ERROR_INI_FILE=37 если файл или
  // задача не найдены, либо параметры задачи некорректны.
  function ReadPropertyEx(device : ppointer;lpFileName : pointer;lpAppName : pointer): cardinal;  stdcall;
  // Функция через параметры передает информацию о размере готового буфера, который можно считывать из модуля, и статистику передачи.
  // Параметры функции:
  // device – хендл ТМ модуля
  // dSizeBuf – адрес переменной, в которую будет записан размер готового буфера в байтах.
  // dwSizeLost – адрес переменной, которую будет записано число пропущенных байт.
  // dwSizeOverflow – адрес переменной, в которую будет записан размер переполнения в байтах.
  function GetReadBufSize(device : ppointer; pSizeBuf : PLongint; pSizeLost : PLongint; pSizeOverflow : PLongint) : cardinal;  stdcall;
  // Функция через параметры передает информацию о размере готового буфера, который можно записать в модуль и статистику передачи.
  // Параметры функции:
  // device – хендл ТМ модуля
  // dSizeBuf – адрес переменной, в которую будет записан размер буфера для приема в байтах.
  // dwSizeLost – адрес переменной, которую будет записано число пропущенных байт.
  // dwSizeOverflow – адрес переменной, в которую будет записан размер переполнения в байтах.
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0).
  function GetWriteBufSize(device : ppointer; pSizeBuf : PLongint; pSizeLost :PLongint;
                           pSizeOverflow : PLongint): cardinal;  stdcall;
  // Функция считывает готовый буфер из ТМ модуля
  // В качестве параметров функции передаются:
  // device – хендл ТМ модуля
  // SizeBuf – адрес переменной, в которой хранится размер буфера в байтах. Значение этой переменной возвращает функция GetReadBufSize.
  // pBuf – адрес буфера, в который будет записана информация.
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0).
  function ReadBuf(device : ppointer; pBuf : PWord; dSizeBuf : PLongint): cardinal;  stdcall;
  // Функция записывает буфер в ТМ модуль.
  // В качестве параметров функции передаются:
  // device – хендл ТМ модуля
  // SizeBuf – адрес переменной, в которой хранится размер записываемого буфера в байтах.
  // pBuf – адрес буфера из которого будет записываться информация.
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0).
  function WriteBuf(device : ppointer; pBuf : PWord; dSizeBuf : PLongint): cardinal;  stdcall;
  // устанавливает обработчик прерывания, поступающего от ТМ модуля после запуска задачи. Прерывание возникает при необходимости приема или передачи очередной порции данных. Функция-обработчик прерывания должна быть статической (иметь фиксированный адрес).
  // В качестве параметров функции передаются:
  // device – хендлер ТМ модуля
  // DataHandler – указатель на объект, который передается в качестве параметра прерывания.
  // IntHandler – адрес функции - обработчика прерывания
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0).
  function SetInterrupt(device : ppointer; DataHandler : pointer; IntHandler : pointer): cardinal;  stdcall;
  // Функция осуществляет сброс статистики. Обнуляются различные счетчики, отображающие статистику (в том числе - число потерянных байт, число переданных байт  и величина переполнения).
  // В качестве параметра функции передается хендл модуля.
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0).
  function ResetCount(device : ppointer): cardinal;  stdcall;
  // Функция определяет размер максимально возможного буфера в словах и записывает
  // в переменную, которой передается в качестве параметра dwMaxBufSize
  function GetMaxBufSize(device : ppointer;dwMaxBufSize : PLongint): cardinal;  stdcall;
  // Функция считывает текущее значение параметра настройки ТМ модуля с идентификатором, передаваемым с помощью параметра Prop, в параметр Variant. Параметр Variant может иметь строковый, вещественный или целый тип в зависимости от идентификатора параметра.
  // Идентификаторы параметров настройки описаны в файле const.h (С++) или MxxxxTypes.pas (Object Pascal). Наборы параметров, соответствующих специфике различных ТМ модулей, отображены в файле Plat_prm.h (C++) или Plat_prm.pas (Object Pascal).
  // В качестве первого параметра функции передается хендл модуля.
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0).
  function GetProperty(device : ppointer; TypeProp:Longint;var VarProp: Variant): cardinal; stdcall;
  // Функция устанавливает параметр настройки ТМ модуля с идентификатором, передаваемым с помощью параметра Prop, в значение, передаваемое с помощью параметра Variant. Параметр Variant должен иметь строковый, вещественный или целый тип в зависимости от идентификатора параметра.
  // Идентификаторы параметров настройки описаны в файле const.h (С++) или MxxxxTypes.pas (Object Pascal). Наборы параметров, соответствующих специфике различных ТМ модулей, отображены в файле Plat_prm.h (C++) или Plat_prm.pas (Object Pascal).
  // В качестве первого параметра функции передается хендл модуля.
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0).
  ...
  // Тестирование ТМ модуля. Хендл модуля передается в качестве первого параметра. lpFileName – путь и имя INI-файла из которого будет считана конфигурация для тестирования, имя которой задается параметром lpAppName.
  // Функция возвращает код успешного завершения (ERROR_NOERROR = 0) в случае если ошибки при тестировании не обнаружены. В противном случае функция возвращает коды ошибок. Код ошибки ERROR_INI_FILE=37 если файл INI или указанная задача не найдены, либо параметры задачи некорректны. Код ошибки ERROR_TEST_PM=65 если произошла ошибка записи/чтения памяти программы ТМ модуля через память ввода/вывода. Код ошибки ERROR_TEST_DM=64 если произошла ошибка записи/чтения памяти данных ТМ модуля через память ввода/вывода. Код ошибки ERROR_TEST_PM_PCI=67 если произошла ошибка записи/чтения памяти программы ТМ модуля через адреса ввода/вывода PCI. Код ошибки ERROR_TEST_DM_PCI=66 если произошла ошибка записи/чтения памяти данных ТМ модуля через адреса ввода/вывода PCI.
  // В процессе выполнения тестирования запускаются функции Load и Config, описанные выше. Поэтому функция тестирования может возвращать коды ошибок, специфичные для этих функций. Ошибка ERROR_TEST_IO=69 в случае возникновения сопровождается дополнительной информацией, записываемой в параметр dwTest. Эта информация специфична для различных типов ТМ модулей:
  // - M2081 и M1081 -  каждый разряд равный 1 соответствует неисправному разряду на входе или на выходе.
  // - M2181 и M1181 –разряды единичных битов соответствуют номерам тестов, вызвавших ошибку.
  // - M2070 и M1070 – для этих модулей данная ошибка не генерируется, dwTest=0.
  ...
  // осциллографирование с платы M2070
  // в массиве pBuf - последовательность 32-разрядных слов. первые 20 бит хранят информацию о состоянии
  // каналов. Причем первый бит - состояние первого канала, 2торой - второго и т.д.
  ...
  // создание callback на асинхронные события
...
Const
  LO_TIMER      = $4;             //  работа по таймеру
  LO_SYNC       = $8;             //  синхронизация по фронту 
  LO_NEG_FRONT  = $8000;          //  по негативному фронту
