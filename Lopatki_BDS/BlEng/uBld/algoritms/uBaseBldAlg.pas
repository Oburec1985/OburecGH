...
type
  ...
  public
    // выставляется в false для простых алгоритмов вроде тахо или статистики импульсов
    // когда нет априорных данных по ступени и не нужно вычислять число лопаток
    // вообще
    needevalBladeCount:boolean;
    // частота дискретизации (<0 если сигнал x/y)
    SampleRate,
...
begin
  ...
  if (sensorind=-1) then
  begin
    // в обороте нет импульсовю. Отбраковываем оборот
    newsensorind:=-1;
  ...
  begin
    ...
    begin
      ...
      eng.getmessage('По датчику '+sensorslist.GetSensor(i).name+
                    ' отфильтровано '+ formatstr(sstruct[i].droped*100,3)+
                    '% оборотов',c_infoMessage);
    end;
  end;
  if SaveSignals then
  begin
