...
type
  ...
  protected
    // проверяем есть ли хоть одна задача обработки данных в запущеном состоянии
  ...
  public
    ...
    // сбросить событие - включить оджидание данных
    // нельзя вызывать в интерфейсном потоке, чтобы не заклинило кнопки
    function waitDataExchangeEvent: boolean;
    // запустить задачи на обработку очередного файла с нуля. Запускается
    // только если пришло событие что данные обновились
...
var
  ...
  t, activetask: ctask;
begin
  // нельзя вызывать в интерфейсном потоке
...
begin
  // не ждем обработки текущих данных т.к. данные еще не загружались
  SetDataProcEvent;
  cDataThreadMng(dataThreads).m_threadtype := p_mode;
  cDataThreadMng(dataThreads).enabled := True;
  // если нет потоков (демо режим, ставим событие, что данные получены)
...
begin
  ...
  // вызываем чтоб завершить остановку потоковь данных
  // (сброс ожидания обработки данных)
  SetDataProcEvent;
  for i := 0 to TaskList.Count - 1 do
  begin
    t := ctask(TaskList.getobj(i));
...
begin
  ...
  // включаем ожидание обраюотки данных
  DropDataProcEvent;
end;
// сбросить событие - выключить оджидание данных
