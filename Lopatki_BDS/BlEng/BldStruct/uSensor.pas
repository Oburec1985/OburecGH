...
type
  ...
  public
    // подсказка почему датчик херово работает
    hint:string;
    pairs:cbaseobjlist;
...
begin
  ...
  begin
    ...
    begin
      RESULT:=21;
      hint:='Не определено положение на ступени';
      exit;
    end;
    if (stage=nil) then
    begin
      RESULT:=21;
      hint:='не назначена ступень';
      exit;
    end;
    if (taho=nil) then
    begin
      RESULT:=21;
      hint:='не назначен тахо датчик';
      exit;
    end;
    ...
    begin
      ...
      begin
        result:=21;
        hint:='соотношение тахо импульсов к таходатчику не совпадает с числом лопаток';
        exit;
      end;
    end;
  end;
