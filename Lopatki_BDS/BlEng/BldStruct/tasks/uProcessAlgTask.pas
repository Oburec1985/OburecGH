...
type
  ...
  protected
    // вызывается bop функции synchronise - в основном потоке
...
begin
  ...
  begin
    ...
    if checkflag(state,c_Run) then
    begin
      // если не выйти из крит секции то в событии onFinishPlayData в деморежиме
      // будит синхродедлок осн потока, т.к. будит по событию меняться состояние
      // SatrtStopCs в основном потоке
...
begin
  ...
  begin
    ...
    begin
      // если поток остановлен то его можно уничтожать сразу
