...
uses
 ...
 Type
  ...
  private
    // косинус допустимого угла при зуме; в камеру призумливаютс€ объекты попадающие в этот угол
  ...
  public
    ...
    // ѕовернуть камеру вокруг обекта
    ...
    // ќтрисовать камеру (пустышки дл€ обозначени€ положени€ и мишени)
    procedure draw;override;
    // јктивизировать камеру. ¬ сцене может быть только одна активна€ камера
    ...
    // ≈сли камера не активна - делает камеру активной
    constructor create;override;
    // процедура пользовательского перемещени€ камеры. ћожет быть переопределена
    // дл€ реализации камеры другого типа
...
begin
  ...
  glLoadIdentity;
  m:=restm;
  // ”станавливает текущую резульирующую матрицу объекта
  glloadmatrixf(@m);
  gltranslate(0,0,2);
...
begin
  if target=nil then
  begin
    // ¬ытаскивает вектор взгл€да (ось z) из матрицы положени€ объекта
    p:=getSight;
    // ¬озвращает вектор умноженый на скал€р
    p:=scalevectorp3(2,p);
...
begin
  ...
  begin
    // находим точку пересечени€ луча из center (центр баундбокса) с плоскостью положени€ камеры и делаем туда параллельный перенос
    up:=getup;
    ...
    // длина вектора PN (p-позици€, N нормаль из вершины на view)
    // V1 - хранит вектор pos - H
    L:=VectorLengthP3(PH)*cos_max;
    // ычисл€ем нормаль и сохран€ем в V1
