...
powershell.exe -NoLogo -NoProfile -Command "[Console]::Out.WriteLine($env:RECORDER_DEPLOY_PASSWORD)" | "%SSH_EXE%" -T -o BatchMode=no -o NumberOfPasswordPrompts=1 -o ConnectTimeout=10 -o ServerAliveInterval=5 -o ServerAliveCountMax=3 "%SSH_TARGET%" "sudo -S -p '' sh -c 'if ! dpkg -i %REMOTE_DEB%; then apt-get -f install -y && dpkg -i %REMOTE_DEB%; fi; config=/var/opt/mera/RecorderLnx/config/coordinator-client.ini; if grep -q ^BaseUrl= \"$config\"; then sed -i \"s#^BaseUrl=.*#BaseUrl=%COORDINATOR_URL%#\" \"$config\"; else printf \"\\n[Coordinator]\\nBaseUrl=%COORDINATOR_URL%\\n\" >>\"$config\"; fi' && dpkg-query -W -f='package=${Package} version=${Version} status=${db:Status-Status}\n' recorderlnx && test -x /opt/mera/RecorderLnx/RecorderLnx && test -x /opt/mera/RecorderLnx/RecorderHostAgent && test -f /etc/xdg/autostart/recorder-host-agent.desktop && grep -F 'BaseUrl=%COORDINATOR_URL%' /var/opt/mera/RecorderLnx/config/coordinator-client.ini && printf 'host-agent=' && if curl -fsS --max-time 3 http://127.0.0.1:8766/api/v1/status; then exit 0; else pgrep -af '/opt/mera/RecorderLnx/RecorderHostAgent'; fi"
