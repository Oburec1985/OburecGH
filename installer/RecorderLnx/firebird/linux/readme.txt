...
Установщик предлагает три независимых компонента:
...
- NTP-сервер — раздача времени компьютерам локальной сети. Используется
  встроенный в пакет RecorderLnx отдельный Chrony; штатный
  systemd-timesyncd не удаляется и продолжает синхронизировать часы сервера.
...
Перед выбором NTP-сервера установите RecorderLnx версии 0.1.39+ntp1 или новее.
В графическом списке NTP-сервер изначально не выбран. Укажите подсеть
клиентов, рассчитанную по адресу и маске сетевого адаптера сервера.
Для текущих КИПов подходит 192.168.9.0/24.
