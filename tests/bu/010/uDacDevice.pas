...
type
  ...
  *)
  ...
  public
    { Назначить обработчик OnBufferEnd (запись в FOnBufferEnd извне класса через свойство
      в среде Delphi 2010 иногда даёт E2035 на присваивание nil). }
    procedure AssignOnBufferEndHandler(const AHandler: TOnBufferEnd);
    { Снять обработчик, только если он совпадает с переданным (method pointer). }
    procedure DetachOnBufferEndIfEqual(const AHandler: TOnBufferEnd);
...
procedure TDacDevice.AssignOnBufferEndHandler(const AHandler: TOnBufferEnd);
begin
  FOnBufferEnd := AHandler;
end;

procedure TDacDevice.DetachOnBufferEndIfEqual(const AHandler: TOnBufferEnd);
var
  lCur: TMethod;
  lReq: TMethod;
begin
  lCur := TMethod(FOnBufferEnd);
  lReq := TMethod(AHandler);
  if (lCur.Code = lReq.Code) and (lCur.Data = lReq.Data) then
    FOnBufferEnd := nil;
end;
