...
// 5) прописать в DllRegisterServer reg.writestring(string(buffer), 'WPExtPack.IEMeraPlg');
// где WPExtPack - библиотека типов (файл ridll)  IEMeraPlg - имя CoClass из библиотеки
...
begin
  ...
  WINPOS.RegisterImpExp(self, self, 'Файлы mera без ГХ', '*.mera');
 end;
...
begin
  ...
  begin
    ...
    txt := 'Файл формата mera' + #13#10;
    str:=mera.ReadString('Main','Date','_');
    txt := txt + 'Дата: ' + str+ #13#10;
  end
end;
