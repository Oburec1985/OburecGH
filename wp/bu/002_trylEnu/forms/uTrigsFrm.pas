...
constructor TTrigsFrm.Create(AOwner: TComponent);
begin
  inherited;
  TranslateForm(Self);
end;
...
begin
  ...
  begin
    ...
    begin
      showmessage('Уже есть аналогичный trigger');
      tr.destroy;
      break;
    end;
...
begin
  Allowed := True; // Указываем, что данный node можно drag'ать
end;
...
begin
  ...
  if (srctrig.trigtype=c_Trig_Start) or (srctrig.trigtype=c_Trig_Stop)  then
    exit;
  Sender.MoveTo(Sender.focusednode, Node, amAddChildLast, False); // перемещаем node
...
procedure TTrigsFrm.VTree1DragOver(Sender: TBaseVirtualTree; Source: TObject;
  ...
  var Effect: Integer; var Accept: Boolean);
begin
  Accept := Sender.GetNodeAt(Pt.x, Pt.y) <> Sender.focusednode; // Drop'ать node сам на себя нельзя
end;
