...
constructor TFFTInverseFrm.Create(AOwner: TComponent);
begin
  inherited;
  TranslateForm(Self);
end;
...
begin
  result := true;
  // ЕСЛИ node СОДЕРЖИТ УЗЛЫ signalЫ
...
var
  ...
  nd: tNodeData;
  hideChild,
  // Если false то будет отображаться даже если все дочерние itemы спрятаны
  HideThisNode,
