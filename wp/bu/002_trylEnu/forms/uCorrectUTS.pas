...
type
  ...
  public
    ...
    slist:tstringlist;
    // вычислено по triggerному signalу
    resshift:double;
    // старт triggerа в tdatetime
    trigstart:tdatetime;

    NoShiftChan:boolean;
    // индекс в LBox
    itemindex:integer;
    // список каналов которые не ссылаются на UTS
  ...
  private
    ...
    lastfile,
    start,
    // name старта испытания в сикон
    siconName:string;
    // список корректируемых fileов
  ...
  private
    ...
    // sList - список каналов UTS fileов
    // NoUTS - список каналов по которым не записан UTS в mera fileе
    procedure findUts(slist:tstringlist; noUTS:tstringlist; src:csrc);
    procedure ShowUTS(s:selectSrc);
    // заполнить комбобокс (список каналов выбора старта)
    procedure FillCB;
    // скоректировать все UTS на поправку
    procedure CorrectUTS;
  public
    constructor Create(AOwner: TComponent); override;
    procedure Linc(mng:cWPObjMng);
    // display выбраные замеры
...
constructor TCorrectUTSFrm.Create(AOwner: TComponent);
begin
  inherited;
  TranslateForm(Self);
end;
...
var
  s:csrc;
  // хранит список корректируемых uts signalов в выбраных sourceах
  ls:selectsrc;
  i:integer;
begin
  clearsignals;
...
begin
  ...
  begin
    ...
    begin
      //Если прорисовываемая string чётная.
      Brush.Color := clRed;
      FillRect(Rect);
...
begin
  ...
  begin
    ...
    // меняем ключи в дублированом fileе
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        signalsLB.Items.Strings[i]:=src.s.name+' shift='+floattostr(src.resshift);
...
begin
  ...
  // shiftаем каналы без UTS. shift происходит только для тех signalов, которые
  // лежат в папке
  if noUTSShift then
  begin
  ...
  // Коррекция signalов у которых нет x y
...
begin
  ...
  begin
    ...
    utslv.SetSubItemByColumnName('name', signal.s.Name, li);
...
begin
  ...
  SiconName:=ifile.ReadString('Main','SICONNAME','СБРОС_СИ');
