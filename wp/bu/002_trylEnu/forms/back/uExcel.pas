...
const
  ...
  xlPasteAllUsingSourceTheme      =	13    ; // Вставка всего содержимого, используя тему оформления sourceа
  ...
  xlLocationAsNewSheet=1;            //на отдельном новом leafе
  xlLocationAsObject=2;              // Разместить диаграмму leafе с данными
  ...
  // скопировать Excel leafсам в себя, но по значению
...
try
 ...
 if CheckExcelInstall then
   ...
   begin
     MessageBox(0,'Приложение MS Excel не установлено на этом компьютере','error',MB_OK+MB_ICONERROR);
     Result:=false;
   end;
except
  Result:=false;
...
Function RangeObjToAdress(r:OleVariant):string;
begin
  result:=Format('string %d, Колонка %d', [R.Row, R.Column]);
end;
...
// Заливка области построения диаграммы из fileа
Function  BrushPlotAreaFromFile(Name:OleVariant;File_:string):boolean;
begin
...
// Заливка области диаграммы из fileа
Function  BrushChartAreaFromFile(Name:OleVariant;File_:string):boolean;
begin
...
// Размещение диаграммы (на leafе с данными, на отдельном leafе)
Function  SetChartLocation(var name:OleVariant;sheet:OleVariant;xlLocation:integer):boolean;
var eee_:string;
begin
