...
uses
  ...
  uWPproc, uCommonMath, NativeXML, uComponentServises, uExcel,
...
type
  ...
  public
    first: cwpsignal;
    second: cwpsignal;
    // level по прямому датчику
    Lvl1: double;
    // level по обратному датчику
    Lvl2: double;
    // level по прямому датчику
    lo_Lvl1: double;
    // level по обратному датчику
  ...
  public
    ...
    t: sTrig;
    reft: sTrig;
    // Сбойная пара (нелинкованный signal обратной связи)
    refData:boolean;
  end;

  // набор выходных данных где каждому значению прямого канала соответствует value его рефа
  ...
  protected
    procedure clearLV;
    // если грохнули замер на signalы которого была слинкована таблица ChansLV
...
var
  ...
  start: cTrigSetList;
  tmplt,
  // елси таблица импульсов обнуления времени создана в автомате а не вручную (добавлена в report),
...
begin
  ...
  begin
    ...
    if not b then
    begin
      showmessage('Не правильный path к resultу');
      exit;
    end;
  end;
  Process;
  ...
  SetCell(1, 1, 1, 'report');
  SetCell(1, 1, 2, src.merafile.Date);
  SetCell(1, 1, 3, ''''+src.merafile.time);
  SetCell(1, 1, 4, ''''+src.merafile.FileName);
  SetCell(1, 2, c_Col_Name, 'signal');
  SetCell(1, 2, c_Col_Value, 'value');
  if UnitsCB.ItemIndex = 0 then
  begin
    SetCell(1, 2, c_Col_Time, 'time, сек.');
    SetCell(1, 2, c_Col_RefTime, 'time, сек.');
    SetCell(1, 2, c_Col_dT, 'dT, сек.');
  end
  else
  begin
    SetCell(1, 2, c_Col_Time, 'time, мс');
    SetCell(1, 2, c_Col_RefTime, 'time, мс');
  ...
  // нужна check на существование второго leafа (а вдруг работаем не с шаблоном)
  // копирование нужно для того чтобы не испортить report после того как leaf исходных данных будет подменен новым протоколом
...
begin
  ...
  // список каналов для reportа по циклограмме
  ...
  begin
    ...
    showmessage('Не удается создать file, возможно диск не существует');
end;
...
begin
  ...
  begin
    ...
    ShowCommonChannels(length(FilterEdit.Text) > 0, ChanTypeRG.ItemIndex,
      FilterEdit.Text);
    // перелинковываем signalы к ChansLV
    RelincPairs;
  end;
end;
...
begin
  ...
  begin
    ...
    ChansLV.SetSubItemByColumnName('trigger1', floattostr(p.Lvl1), new_li);
  end;
...
begin
  ...
  ChansLV.SetSubItemByColumnName('trigger1', floattostr(cpair(item.data).Lvl1),
    item);
  ChansLV.SetSubItemByColumnName('trigger2', floattostr(cpair(item.data).Lvl2),
    item);
end;
...
begin
  ...
  begin
    ...
    CommonLV.SetSubItemByColumnName('type платы', sname, li);
  end;
...
begin
  ...
  begin
    ...
    begin
      if v < lo then
      begin
        // не нужно искать для signalа обнуления времени
  ...
  begin
    ...
    begin
      ...
      begin
        trig := false;
        lo_trig := True;
        // первое value пишем только для уровня "1"
        if Writefirst then
        begin
...
begin
  ...
  CyclogramRepFrm.Caption:='setup reportа циклограммы +(версия '+datetostr(now)+')';
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if index2<>-1 then
          begin
            // check что полученный импульс соответствиует действительно
    ...
    // дополняем report данными по нелинокванным обратным связям (лишние импульсы)
...
begin
  ...
  ChansLV.SetSubItemByColumnName('trigger1', floattostr(p.Lvl1), li);
  if p.second <> nil then
  begin
    ChansLV.SetSubItemByColumnName('Обратная связь', p.second.name, li);
    ChansLV.SetSubItemByColumnName('trigger2', floattostr(p.Lvl2), li);
  end;
end;
...
begin
  ...
  begin
    ...
    ChansLV.SetSubItemByColumnName('trigger2', floattostr(cpair(li.data).Lvl2),
      li);
  end;
end;
