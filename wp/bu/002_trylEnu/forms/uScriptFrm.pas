...
type
  ...
  ErrorStruct = record
    // string с ошибкой, номер символа с ошибкой
    Row, num: integer;
    // описание ошибки
    dsc: string;
  end;
  ...
  TimeStruct = record
    ...
    min_dT,
    // минимальное time среди обрабатываемых тегов
    minT,
    // мarcимальное time среди обрабатываемых тегов
    maxT: double;
    valid: boolean;
  end;
  ...
  public
    m_pitem: pointer; // ссылка на data
    ffreq: double;
    // переменная для прореживания записи в тег. Записано time последней записи в канал
  ...
  public
    ...
    // методо получения описания typeа
    ...
    function Invoke(DispID: integer; // Идентификатор вызываемого метода или свойства, полученный от GetIdsOfNames
      ...
      // DISPATCH_PROPERTYGET Запрашивается value свойства
      // DISPATCH_PROPERTYPUT Устанавливается value свойства
      ...
      // и количества itemов в этих массивах. Параметры передаются в порядке, обратном их порядку следования в функции, как
      // принято в Visual Basic
      var Params; VarResult, // Адрес переменной typeа OleVariant, в которую должен быть помещен result callа метода или value свойства или NIL, если возвращаемое value не требуется.
      ...
      // При callе Invoke не осуществляется никаких проверок, поэтому при его самостоятельной реализации необходимо соблюдать аккуратность при работе с переданными адресами массивов и переменных.
  ...
  private
    ...
    // folder с resultами
    resSrc: csrc;
    // список tScriptItem-ов из sourceа данных
    srclist,
  ...
  private
    ...
    // clear движка от созданых тегов
    procedure ClearTags;
  ...
  protected
    ...
    // функция получает список тегов в коде скрипта в которые пишутся data
    ...
    // list используется только как список строковых имен. по именам ищутся signalы в списке srcList
...
uses
  uLocalizationHelper, uWpExtPack, uAddScriptItemFrm;
{$R *.dfm}
...
begin
  ...
  begin
    ...
    begin
      if result = '' then
      begin
        // name переменной не может начинаться с '_' поэтому в качестве первой скобки добавляем A_A
...
// Найти первое попавшееся name в скрипте ограниченное символами "{}"
...
begin
  ...
  // если требуется установить value свойства, то...
  if ((Flags and DISPATCH_PROPERTYPUT) = DISPATCH_PROPERTYPUT) then
  begin
    // если указано в аргументе новое value, то...
    if (DISPPARAMS(Params).cArgs <> 0) then
    begin
      // получение значения, check и при
      // необходимости смена typeа значения
  ...
  // если требуется получить value свойства, то...
  else
  begin
...
begin
  ...
  begin
    ...
    // защита от повторного добавления signalа
...
// leaf должен запрещать повторы и быть отсортированным
...
begin
  ...
  begin
    ...
    // ищем знак равно или другие operatorы
    if findname then
    begin
      while findname do
      begin
...
begin
  ...
  li.data := si;
  ListViewItems.SetSubItemByColumnName('name', si.Name, li);
  if si.ScriptChannel then
  begin
    ListViewItems.SetSubItemByColumnName('fs', floattostr(si.freq), li);
    ListViewItems.SetSubItemByColumnName('type', 'Результирующий', li);
  end
  else
  begin
    ...
    begin
      ListViewItems.SetSubItemByColumnName('fs', floattostr(si.freq), li);
      ListViewItems.SetSubItemByColumnName('type', 'Замер', li);
    end
    else
    begin
      ListViewItems.SetSubItemByColumnName('fs',
        floattostr(1 / ScriptDT.FloatNum), li);
      ListViewItems.SetSubItemByColumnName('type', 'Виртуальный', li);
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // требуется доработать для не линкованого signalа
...
begin
  ...
  // задержка после которой генерится error - 5 секунд
