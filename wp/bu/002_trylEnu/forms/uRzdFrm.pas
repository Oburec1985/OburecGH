...
type
  ...
  TEi = record
    sname, // name датчика по которому построен
    fname, // name силы по которому построен
    // ссылка на узлы в дереве WP
    nCloud, nPoly: iwpnode;
    cloud, poly: iwpSignal;
    str_sname, str_fName: string;
    fmax: double;
    // value компонента вектора f
    ei: double;
  ...
  public
    ...
    // 3 последних восстановленных signalа используется при построении ф-ции сравнения
    lastn1, lastn2, lastn3: string;
    n1, n2, n3: iwpnode;
    // signalы по которым посчитана матрица
    ...
    // степень полинома
    poly: integer;
    // последние посчитанные signalы
    resVt, resGt: iwpSignal;
  public
    // возвращает name в ини fileе
    ...
    // возвращает приставку к имени signalа вида _1_reg_01
  ...
  public
    ...
    // текущий source signalа для обработки
    m_src,
    // sourceи соответствующие нагружениям
    vc_src, h_src, hout_src, hin_src: csrc;
    // секция ини fileа в которую сохраняются настройки и path к ини fileу
    m_section, m_cfg, m_VCPref, m_H_Pref, m_Hin_Pref, m_Hout_Pref: string;

    m_wpMng: cwpobjmng;
    // graphа для выбора участков времен по курсорам
    ...
    m_F2zFyValue, m_F2zFyMaxX: double; // value полинома Fz(Fy)
  ...
  protected
    ...
    // Заполняем комбобоксы доступными signalами
    ...
    // заполняем пути к signalам калибровки
    ...
    // построить сравннеие двух signalов по формуле
    // s1-s2/f(base,f);
    ...
    // возвращает СОЗДАННЫЙ ВНОВЬ стринг leaf содержащий имена датчиков соотносящиеся с сечением
    ...
    function GetSensorsNameWithPref(mf: string; sect: integer): tstringlist;
      overload;
    // load данных для построения матрицы
  ...
  public
    ...
    // filename - path к КАТАЛОГУ с мера fileом
...
var
  ...
  i: integer;
begin
  // path к замеру для тарировки вертикально центрального нагружения
...
begin
  ...
  // process свойства имен датчиков
  for i := 0 to MatrixLV.Items.count - 1 do
  begin
...
begin
  ...
  if Abs(vt.MinY) > Abs(vt.MaxY) then
    trigval := -trigval;
  // создаем страницу с triggerным уровнем
  graph := createline(vt);
  ...
  // создаем graphи калибровочной матрицы
  ...
  begin
    ...
    begin
      ...
      begin
        JournalLB.AddItem('Не найден result тарировки Vt' + ' Сечение:' +
            inttostr(m.cut) + ' Регион:' + m.region + ' в signalе ' +
...
begin
  ...
  // создаем страницу reportа в памяти
...
begin
  ...
  m_log.addInfoMes('load fileа успешна');
  // укорачиваем путть с конца на 1 level
  str := TrimPath(str);
  ...
  m_log.addInfoMes('process матриц - успешно');
  ...
  m_log.addInfoMes('получение каталога результирующего signalа');
  if m=nil then
  begin
  ...
  //begin
  //  m_log.addErrorMes('Не посчитан signal VT');
  //  exit;
  //end;
  ...
  m_pageIndex := 0;
  m_log.addInfoMes('до построения reportа');
  for i := 0 to MatrixList.count - 1 do
  begin
    m := MatrixList.Items[i];
  ...
  m_log.addInfoMes('после построения reportа');
...
begin
  ...
  if DataObject.GetData(FmtEtc, Medium) = S_OK then
    try
      try
        // второй параметр iFile - при ffffff - number fileов, иначе индекс fileа
...
begin
  ...
  ExportMatrixPath.Hint := 'path: ' + str;
end;
...
begin
  ...
  str := dir + '\' + extractfilename(str);
  ImportMatrixPath.Hint := 'path: ' + str;
  // ищем file
...
begin
  ...
  // создаем апроксимирующий полином в дереве signalов
...
begin
  ...
  begin
    ...
    DrawGraphBtn.Hint := 'Не загружены signalы';
  end
  else
  begin
    CreateMatrixBtn.Caption := 'Создать';
    LoadBtnPanel.color := clGreen;
    DrawGraphBtn.Hint := 'Отрисовать graphи';
  end;
end;
...
begin
  ...
  begin
    JournalLB.AddItem('interval signalа не совпадает с выбраным intervalом!' +
...
begin
  ...
  interval1 := interval;
  JournalLB.AddItem('interval: ' + formatstrNoE(interval.x,
...
begin
  ...
  // res1 - signal разница между исходным и расчитанным
  p := createline(res1, p.hpage);
  // res - signal сравнение
...
procedure TRZDFrm.ApplyMatrix(s1, s2, s3, s4: iwpSignal; p_g: d2array;
  srcpath: string; sname: string; m: TRZDMatrix);
var
  // максимальная частота опроса среди signalов
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          // проверяем вышли мы за level гистерезиса или нет
          if findtrg then
          begin
            if V + noise > lvl then
            begin
  ...
  // вычисляем минимальное value импульса signalа
  min := Abs(s.MaxY);
  max := -1;
  ...
  // вычисляем доверительный interval
  // цена деления одного импульса
  // v:=1/res.Count;
  // импульс на котором мы еще меньше доверительного intervalа
  V := 1 / res1.count;
...
begin
  list := tstringlist.create;
  // сканируемый каталог, маска, список resultов
...
begin
  ...
  begin
    ...
    begin
      RZDFrm.JournalLB.AddItem(
        'Не определен один из signalов, попытка поиска по префиксам...', nil);
      exit;
    end;
  end
  else
  ...
  begin
    RZDFrm.JournalLB.AddItem('Не найдены signalы', nil);
  end;
end;
