...
// алгоритм не знает списка signalов обработки (он получает по одному
  // signalу перед callом). Работу с группой signalов реализует форма
...
constructor TCounterFrm.Create(AOwner: TComponent);
begin
  inherited;
  TranslateForm(Self);
end;
