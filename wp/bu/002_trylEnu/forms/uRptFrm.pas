...
type
  ...
  public
    ...
    // type 0 -физ, 1 - индексы, 2 - triggerы
    bandType: integer;
  private
    fTrig1, ftrig2: ctrig;
  protected
  ...
  private
    eo: TObject;
    mng: cWPObjMng;
    // невозможно создать выходной file
    invalidpath: Boolean;
    selectInd: integer;
  private
    ...
    // если file для обработки алгоритмом не ссылается на мера file,
    // то path должен быть абсолютным
...
const
  ...
  c_SPMMax_Name = 'Максимальное value в спектре';
...
begin
  ...
  begin
    li := lv.Items[I];
    lv.GetSubItemByColumnName('name', li, str);
    if str = param then
    begin
      lv.GetSubItemByColumnName('value', li, str);
      result := str;
      exit;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        showmessage('file конфигурации reportа поврежден');
        exit;
      end;
      // type 0 -физ, 1 - индексы, 2 - triggerы
...
var
  str: string;
begin
  ParamsPropLV.GetSubItemByColumnName('name', Item, str);
  ParamNameEdit.Text := str;
  ParamsPropLV.GetSubItemByColumnName('value', Item, str);
...
begin
  ...
  ParamsPropLV.SetSubItemByColumnName('value', ParamValEdit.Text, li);
...
begin
  ...
  begin
    ...
    ParamsPropLV.SetSubItemByColumnName('name', 'Название', li);
    ParamsPropLV.SetSubItemByColumnName('value', str, li);
    li := ParamsPropLV.Items.Add;
    ParamsPropLV.SetSubItemByColumnName('name', 'F1', li);
    ParamsPropLV.SetSubItemByColumnName('value', floattostr(a), li);
    li := ParamsPropLV.Items.Add;
    ParamsPropLV.SetSubItemByColumnName('name', 'F2', li);
    ParamsPropLV.SetSubItemByColumnName('value', floattostr(b), li);
  end;
  ...
  begin
    ...
    ParamsPropLV.SetSubItemByColumnName('value', str, li);
    li := ParamsPropLV.Items.Add;
    ParamsPropLV.SetSubItemByColumnName('name', 'Lvl1', li);
    ParamsPropLV.SetSubItemByColumnName('value', floattostr(a), li);
    li := ParamsPropLV.Items.Add;
    ParamsPropLV.SetSubItemByColumnName('name', 'Lvl2', li);
    ParamsPropLV.SetSubItemByColumnName('value', floattostr(b), li);
    li := ParamsPropLV.Items.Add;
    ParamsPropLV.SetSubItemByColumnName('name', 'Проц.', li);
    ParamsPropLV.SetSubItemByColumnName('value', inttostr(round(c)), li);
  end;
  ...
  begin
    ...
    ParamsPropLV.SetSubItemByColumnName('value', str, li);
    li := ParamsPropLV.Items.Add;
    ParamsPropLV.SetSubItemByColumnName('name', 'T', li);
    ParamsPropLV.SetSubItemByColumnName('value', floattostr(a), li);
    li := ParamsPropLV.Items.Add;
    ParamsPropLV.SetSubItemByColumnName('name', 'FFTNum', li);
    ParamsPropLV.SetSubItemByColumnName('value', inttostr(i), li);
  end;
end;
...
begin
  ...
  // signal разбивается на порции, для каждой порции строится спектр и запоминается максимальная составляющая спектра
  // в report попадает максимальное value спектра из всего signalа, time порции в которой найден максимум и частота
  ParamsLB.AddItem('Максимальное value в спектре', nil);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      // важно писать полный path, т.к. по нему потом определяется source
...
begin
  // копирование настроек формы в operator
  ApplyOpts;
  str := GetPropertyStr;
...
begin
  ...
  begin
    ...
    begin
      UseMDBCB.ShowHint:=true;
      UseMDBCB.Hint:='path к БДИ не найден';
...
begin
  ...
  if str = 'Максимальное value в спектре' then
  begin
