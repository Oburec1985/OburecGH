...
type
  ...
  private
    ...
    m_ifile, m_section,
    // префикс для имен сохраняемых fileов
    m_key,
    // path к плагину
    m_startdir:string;
    // дефолтное name каталога и fileа по умолчанию
    defName, defFolder,
    // path на основании которого строяться относительныен пути
    objectName: string;
  private
  ...
  public
    ...
    // установить path по которому будут сохраняться настройки
    Procedure SetIFile(fname, section, key, startdir:string);
    procedure SaveParams;
    procedure LoadParams;
    // вернуть path к шаблону
...
begin
  ...
  TmpltEdit.Text := ifile.ReadString(m_section, m_key+'Temlate','report циклограмма.xls');
  TmpltEditChange(nil);
end;
...
begin
  case NameRG.ItemIndex of
    0:
      begin
        NameRG.Caption := 'name замера по умолчанию';
        GenName;
      end;
    1:
      begin
        NameRG.Caption := 'name замера';
