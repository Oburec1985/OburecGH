...
begin
  ...
  begin
    ...
    begin
      lv1.GetSubItemByColumnName('type', lv1.items[i], str);
      itemsp:=Pars(str);
      ...
      str:=GenTypeStr(p);
      li:=lv1.Items[i];
      lv1.SetSubItemByColumnName('type',str, li);
    end;
  end;
end;
...
begin
  ...
  begin
    str:=list.strings[i];
    li:=lv1.items.Add;
    LV1.SetSubItemByColumnName('file',str,li);
    r.sType:=0;
...
begin
  ...
  begin
    ...
    if li.Selected then
    begin
      LV1.GetSubItemByColumnName('file', li, str);
      SetMultiSelectComponentString(signame,str);
      str:='';
      capt:=str;
      LV1.GetSubItemByColumnName('type', li, str);
      if str<>'' then
      begin
        p:=Pars(str);
...
begin
  ...
  begin
    ...
    if p=nil then
    begin
      showmessage('Не назначен регион/ сечение signalу: '+str);
      exit;
    end;
    ...
    case p.pars.sType of
      ...
      begin
        ...
        begin
          ...
          showmessage('Нельзя назначить одному региону и сечению два замера одного typeа '+str);
          exit;
        end;
        parslist.Add(p);
      end;
