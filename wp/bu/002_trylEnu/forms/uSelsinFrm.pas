...
constructor TSelsinFrm.Create(AOwner: TComponent);
begin
  inherited;
  TranslateForm(Self);
end;
...
begin
  ...
  str:=ParsToStr(pars);
  pars.Destroy;
  oper:= WP.GetObject('/VibroOpers/Последовательная process (тренды)') as IWPOperator;
  if (Assigned(oper)) then
  begin
...
begin
  ...
  ChansLV.SetSubItemByColumnName('name',s.name,li);
  ...
  ChansLV.SetSubItemByColumnName('без общ. тчк.', booltostr(s.commonPoint), li);
  if s.bSelsin then
    ChansLV.SetSubItemByColumnName('type', 'SinCos', li)
  else
    ChansLV.SetSubItemByColumnName('type', 'Сельсин', li);
end;
...
begin
  ...
  begin
    ...
    begin
      // если error второго меньше ошибки первого
...
var
  ...
  // угол для проверок
  a_s03,a_s14,a_s25,
  // сохраняем value угла для следующего шага
  res:double;
...
begin
  ...
  begin
    ...
    // Вычисляем конечное value угла по таблице квадрантов
    ...
    // пишем в s03 самый точный result
...
begin
  //oper:= WP.GetObject('/VibroOpers/Последовательная process (тренды)') as IWPOperator;
  //wp.Link('VibroOpers','Последовательная process (тренды)',oper);
  //wp.Refresh;
  CheckList;
