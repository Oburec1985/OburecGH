...
type
  ...
  public
    ...
    slist:tstringlist;
    // вычислено по триггерному сигналу
    resshift:double;
    // старт триггера в tdatetime
    trigstart:tdatetime;
  ...
  private
    ...
    lastfile,
    start,
    // имя старта испытания в сикон
    siconName:string;
    // список корректируемых файлов
  ...
  private
    ...
    // sList - список каналов UTS файлов
    // NoUTS - список каналов по которым не записан UTS в mera файле
...
var
  s:csrc;
  // хранит список корректируемых uts сигналов в выбраных источниках
  ls:selectsrc;
  i:integer;
begin
  clearsignals;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        signalsLB.Items.Strings[i]:=src.s.name+' Сдвиг='+floattostr(src.resshift);
...
begin
  ...
  // сдвигаем каналы без UTS. Сдвиг происходит только для тех сигналов, которые
  // лежат в папке
  if noUTSShift then
  begin
  ...
  // Коррекция сигналов у которых нет x y
...
begin
  ...
  begin
    ...
    utslv.SetSubItemByColumnName('Имя', signal.s.Name, li);
