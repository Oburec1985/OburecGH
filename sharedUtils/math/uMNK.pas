...
type
  // первый компонент строка, второй столбец m[r,c]
  d2array = array of array of double;
  // матрицы записываютс€ по столбцам, т.е. Vcol1 ,..Vcoln где вектор компонентов состо€щий из
  // элементов row m[0] = колонка V0
  // “ранспонирование
  ...
  // size - число точек в массиве
  procedure buildMNK(m:integer; x, y: olevariant; size:integer; var сi:array of double);overload;
  procedure buildMNK(m:integer; y: olevariant; x0,dx:double; size:integer; var сi:array of double);overload;
...
begin
  ...
  // цикл по строкам первой матрицы
  for i:=0 to m1d2-1 do
  begin
    // цикл по колонкам второй матрицы
