...
// Удалить на конце строки символы не являющиеся цифрами
...
Function CheckHex(str:string):boolean;
 ...
 // Функция для поиска минимального и максимального значения в массиве Double.
 // Параметры:
 //   Data: Входной динамический массив значений Double.
 //   MinVal: Выходной параметр. Здесь будет записано минимальное значение, найденное в массиве.  //   MaxVal: Выходной параметр. Здесь будет записано максимальное значение, найденное в массиве.  // Возвращает True, если массив не пуст и поиск успешно выполнен, иначе False.
 function FindMinMaxDouble(const Data: array of double; var MinVal, MaxVal: Double): Boolean;
 // Функция для поиска максимального значения и его индекса в массиве Double.
  // Параметры:
  //   Data: Входной динамический массив значений Double.
  //   MaxVal: Выходной параметр. Здесь будет записано максимальное значение, найденное в массиве.
  //   MaxIndex: Выходной параметр. Здесь будет записан индекс элемента, содержащего MaxVal.
  //             Если массив пуст, MaxIndex будет равен -1.
  // Возвращает True, если массив не пуст и поиск успешно выполнен, иначе False.
...
var
  i, mini, maxi: Integer;
begin
  // Изначально предполагаем, что массив пуст или некорректен
  Result := False;
  MinVal := MaxDouble; // Инициализируем минимальное значение максимально возможным
  MaxVal := -MaxDouble; // Инициализируем максимальное значение минимально возможным

  // Проверяем, не пуст ли входной массив
  if Length(Data) = 0 then
  begin
    Exit; // Если массив пуст, выходим, возвращая False
  end;

  // Инициализируем MinVal и MaxVal первым элементом массива.
  // Это важно, чтобы корректно обработать массивы из одного элемента
  // и избежать проблем, если все значения будут, например, отрицательными.
  MinVal := Data[Low(Data)];
  MaxVal := Data[Low(Data)];

  // Перебираем остальные элементы массива, начиная со второго
  ...
  Result := True; // Если мы дошли до сюда, значит, массив не пуст и значения найдены
end;
...
begin
  // Изначально предполагаем, что массив пуст или некорректен
  Result := False;
  MaxVal := -MaxDouble; // Инициализируем максимальное значение наименьшим возможным double
  MaxIndex := -1;       // Инициализируем индекс максимума как -1 (для пустого массива)
  ...
  // Инициализируем MaxVal и MaxIndex первым элементом массива.
  // Это важно, чтобы корректно обработать массивы из одного элемента
  // и избежать проблем, если все значения будут, например, отрицательными.
  MaxVal := Data[Low(Data)];
  MaxIndex := Low(Data);
  ...
  for i := Low(Data) + 1 to High(Data) do
  begin
    // Если текущий элемент больше текущего максимума, обновляем максимум и его индекс
    if Data[i] > MaxVal then
    begin
      MaxVal := Data[i];
      MaxIndex := i;
  ...
  Result := True; // Если мы дошли до сюда, значит, массив не пуст и максимум найден
end;
