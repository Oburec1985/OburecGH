...
type
  ...
  protected
    ...
    // вьюпорт в который рисуются линии и текст(простой вьюпорт за вычетом скролл баров)
    DrawViewport:array[0..3] of integer;
    // высота строк
  ...
  protected
    // ширина колонки
    fwidth:integer;
    // вьюпорт колонки с учетом отступов
    viewport:array[0..3] of glint;
    // масштаб текста колонки
    TextScale:point2;
    // дополнительное смещение текста связанное со сдвигом области просмотра
    // в таблице
    i_XTextOffset:integer;
    f_XTextOffset:single;
  protected
...
begin
  m_iRawHeigth:=H;
  m_fRawHeigth:=H*(fmax.y-fmin.y)/DrawViewport[3];
  rawcount:=trunc(DrawViewport[3]-m_iRawHeigth/m_iRawHeigth);
...
begin
  ...
  begin
    ...
    begin
      ...
      col.i_XTextOffset:=pixOffset;
    end;
    col.viewport[1]:=h;
    col.viewport[3]:=DrawViewport[3];
    font:=GetFont(fFontIndex);
    col.TextScale:=font.EvalScale(col.viewport[2],m_iRawHeigth);
    w:=w+col.width;
  end;
end;
...
begin
  inherited;
  DrawViewport[0]:=m_viewport[0];
  DrawViewport[1]:=m_viewport[1];
  DrawViewport[2]:=m_viewport[2]-scrollBoxH.m_iWidth;
  DrawViewport[3]:=m_viewport[3]-scrollBoxV.m_iWidth;
