...
const
  // ѕроисходит при удалении объекта. ¬ качестве параметра Sender передаетс€ удал€емый объект
  E_OnDestroyObject = $00000040;
  // происходит при призумливании
  E_OnZoom = $00000080;
  // происходит при движении мыши
  E_OnMouseMove = $00000100;
  // происходит при перерисовке страницы
  e_onDraw = $00000200;
  // ¬ызов после SwapBuffers здесь корректно считать картинку
  ...
  // происходит при перерисовке страницы
  e_onResize = $00001000;
  // происходит при линковке компонента к новому родителю, в том числе к nil
  e_onLincParent = $00002000;
  // происходит при добавлении тренда. в качестве sensder сам тренд
  e_onUpdateBound = $00004000;
  // происходит при изменении масштаба оси
  e_OnChangeAxisScale = $00008000;
  // происходит при изменении имени объекта который передаетс€ в качестве sender
