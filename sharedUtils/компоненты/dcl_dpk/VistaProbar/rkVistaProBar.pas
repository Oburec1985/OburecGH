...
procedure SmoothGradient(Canvas: TCanvas; const ARect: TRect;
  const c1: TColor; const Gray: Boolean);
type
  PRGB = ^TRGB;
...
var
  ...
  bmp: TBitmap;
begin
  if ((ARect.Right - ARect.Left) - 1 <= 0) or ((ARect.Bottom - ARect.Top) - 1 <=
    1) then
    exit;
  bmp := TBitmap.Create;
...
procedure TVistaProBar.Draw(Canvas: TCanvas; Pos, Max: integer);
var
  R: TRect;
  Len: integer;
  Percent: Extended;
