// CodeGear C++Builder
// Copyright (c) 1995, 2009 by Embarcadero Technologies, Inc.
// All rights reserved

// (DO NOT EDIT: machine generated header) 'Jcldevelopertools.pas' rev: 21.00

#ifndef JcldevelopertoolsHPP
#define JcldevelopertoolsHPP

#pragma delphiheader begin
#pragma option push
#pragma option -w-      // All warnings off
#pragma option -Vx      // Zero-length empty class member functions
#pragma pack(push,8)
#include <System.hpp>	// Pascal unit
#include <Sysinit.hpp>	// Pascal unit
#include <Jclcompilerutils.hpp>	// Pascal unit
#include <Jcldevtoolsresources.hpp>	// Pascal unit
#include <Jclideutils.hpp>	// Pascal unit
#include <Jclpreprocessoralgorithmstemplates.hpp>	// Pascal unit
#include <Jclpreprocessorarrayliststemplates.hpp>	// Pascal unit
#include <Jclpreprocessorarraysetstemplates.hpp>	// Pascal unit
#include <Jclpreprocessorbinarytreestemplates.hpp>	// Pascal unit
#include <Jclpreprocessorcontainer1dtemplates.hpp>	// Pascal unit
#include <Jclpreprocessorcontainer2dtemplates.hpp>	// Pascal unit
#include <Jclpreprocessorcontainerintftemplates.hpp>	// Pascal unit
#include <Jclpreprocessorcontainerknownmaps.hpp>	// Pascal unit
#include <Jclpreprocessorcontainerknowntypes.hpp>	// Pascal unit
#include <Jclpreprocessorcontainertemplates.hpp>	// Pascal unit
#include <Jclpreprocessorcontainertypes.hpp>	// Pascal unit
#include <Jclpreprocessorexcdlgtemplates.hpp>	// Pascal unit
#include <Jclpreprocessorhashmapstemplates.hpp>	// Pascal unit
#include <Jclpreprocessorlexer.hpp>	// Pascal unit
#include <Jclpreprocessorlinkedliststemplates.hpp>	// Pascal unit
#include <Jclpreprocessorparser.hpp>	// Pascal unit
#include <Jclpreprocessorqueuestemplates.hpp>	// Pascal unit
#include <Jclpreprocessorsortedmapstemplates.hpp>	// Pascal unit
#include <Jclpreprocessorstackstemplates.hpp>	// Pascal unit
#include <Jclpreprocessortemplates.hpp>	// Pascal unit
#include <Jclpreprocessortreestemplates.hpp>	// Pascal unit
#include <Jclpreprocessorvectorstemplates.hpp>	// Pascal unit
#include <Jclusesutils.hpp>	// Pascal unit
#include <Jclhelputils.hpp>	// Pascal unit
#include <Jclmsbuild.hpp>	// Pascal unit
#include <Mshelpservices_tlb.hpp>	// Pascal unit
#include <Variants.hpp>	// Pascal unit
#include <Windows.hpp>	// Pascal unit
#include <Sysutils.hpp>	// Pascal unit
#include <Varutils.hpp>	// Pascal unit
#include <Typinfo.hpp>	// Pascal unit
#include <Classes.hpp>	// Pascal unit
#include <Timespan.hpp>	// Pascal unit
#include <Syncobjs.hpp>	// Pascal unit
#include <Jclresources.hpp>	// Pascal unit
#include <Jclbase.hpp>	// Pascal unit
#include <Jclwin32.hpp>	// Pascal unit
#include <Jcllogic.hpp>	// Pascal unit
#include <Ansistrings.hpp>	// Pascal unit
#include <Jclstringconversions.hpp>	// Pascal unit
#include <Jclcharsets.hpp>	// Pascal unit
#include <Jcl8087.hpp>	// Pascal unit
#include <Jclmath.hpp>	// Pascal unit
#include <Jclstreams.hpp>	// Pascal unit
#include <Jclansistrings.hpp>	// Pascal unit
#include <Snmp.hpp>	// Pascal unit
#include <Comobj.hpp>	// Pascal unit
#include <Jclsecurity.hpp>	// Pascal unit
#include <Jcldatetime.hpp>	// Pascal unit
#include <Jclfileutils.hpp>	// Pascal unit
#include <Jclshell.hpp>	// Pascal unit
#include <Ioutils.hpp>	// Pascal unit
#include <Inifiles.hpp>	// Pascal unit
#include <Jclinifiles.hpp>	// Pascal unit
#include <Jclsysinfo.hpp>	// Pascal unit
#include <Jclunicode.hpp>	// Pascal unit
#include <Jclwidestrings.hpp>	// Pascal unit
#include <Jclstrings.hpp>	// Pascal unit
#include <Jclregistry.hpp>	// Pascal unit
#include <Jclsynch.hpp>	// Pascal unit
#include <Jclconsole.hpp>	// Pascal unit
#include <Jclsysutils.hpp>	// Pascal unit
#include <Jclunitversioning.hpp>	// Pascal unit
#include <Registry.hpp>	// Pascal unit
#include <Jclpeimage.hpp>	// Pascal unit
#include <Jcltd32.hpp>	// Pascal unit
#include <Jclhookexcept.hpp>	// Pascal unit
#include <Jcldebug.hpp>	// Pascal unit
#include <Jclsimplexml.hpp>	// Pascal unit
#include <Mscoree_tlb.hpp>	// Pascal unit
#include <Mscorlib_tlb.hpp>	// Pascal unit
#include <Jcldotnet.hpp>	// Pascal unit
#include <Jclcontainerintf.hpp>	// Pascal unit
#include <Jclstrhashmap.hpp>	// Pascal unit
#include <Jclalgorithms.hpp>	// Pascal unit
#include <Jclabstractcontainers.hpp>	// Pascal unit
#include <Jclarraylists.hpp>	// Pascal unit
#include <Jclarraysets.hpp>	// Pascal unit
#include <Jclhashmaps.hpp>	// Pascal unit
#include <Jclstacks.hpp>	// Pascal unit
#include <Jclnotify.hpp>	// Pascal unit
#include <Graphics.hpp>	// Pascal unit
#include <Uxtheme.hpp>	// Pascal unit
#include <Multimon.hpp>	// Pascal unit
#include <Actnlist.hpp>	// Pascal unit
#include <Controls.hpp>	// Pascal unit
#include <Themes.hpp>	// Pascal unit
#include <Menus.hpp>	// Pascal unit
#include <Helpintfs.hpp>	// Pascal unit
#include <Flatsb.hpp>	// Pascal unit
#include <Printers.hpp>	// Pascal unit
#include <Graphutil.hpp>	// Pascal unit
#include <Extctrls.hpp>	// Pascal unit
#include <Comctrls.hpp>	// Pascal unit
#include <Dialogs.hpp>	// Pascal unit
#include <Clipbrd.hpp>	// Pascal unit
#include <Forms.hpp>	// Pascal unit
#include <Jclrtti.hpp>	// Pascal unit

//-- user supplied -----------------------------------------------------------

namespace Jcldevelopertools
{
//-- type declarations -------------------------------------------------------
//-- var, const, procedure ---------------------------------------------------

}	/* namespace Jcldevelopertools */
using namespace Jcldevelopertools;
#pragma pack(pop)
#pragma option pop

#pragma delphiheader end.
//-- end unit ----------------------------------------------------------------
#endif	// JcldevelopertoolsHPP
