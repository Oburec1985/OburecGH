...
{                                                                             }
{                                                                             }
{ Модуль fbUtils - интерфейсный модуль для библиотеки функций IBXFBUtils      }
...
begin
  ...
  begin
    ...
    @FBPackagesIsCorrect    := GetProcAddress(fbUtilsHlib, 'FBPackagesIsCorrect');
    ...
    @FBCreateConnection     := GetProcAddress(fbUtilsHlib, 'FBCreateConnection');
    @FBConnectDB            := GetProcAddress(fbUtilsHlib, 'FBConnectDB');
    @FBDisconnectDB         := GetProcAddress(fbUtilsHlib, 'FBDisconnectDB');
    @FBFreeConnection       := GetProcAddress(fbUtilsHlib, 'FBFreeConnection');
    @FBCreateTransaction    := GetProcAddress(fbUtilsHlib, 'FBCreateTransaction');
    @FBCreateDataSet        := GetProcAddress(fbUtilsHlib, 'FBCreateDataSet');
    @FBCreateAndOpenDataSet := GetProcAddress(fbUtilsHlib, 'FBCreateAndOpenDataSet');
    @FBCreateAndOpenTable   := GetProcAddress(fbUtilsHlib, 'FBCreateAndOpenTable');
    @FBExecQuery            := GetProcAddress(fbUtilsHlib, 'FBExecQuery');
    @FBUpdateRecordBase     := GetProcAddress(fbUtilsHlib, 'FBUpdateRecordBase');
    @FBInsertRecordBase     := GetProcAddress(fbUtilsHlib, 'FBInsertRecordBase');
    @FBDeleteRecordBase     := GetProcAddress(fbUtilsHlib, 'FBDeleteRecordBase');
    @FBUpdateOrInsertRecordBase  := GetProcAddress(fbUtilsHlib, 'FBUpdateOrInsertRecordBase');
    @FBUtilsVersion         := GetProcAddress(fbUtilsHlib, 'FBUtilsVersion');
    @FBSetUserName          := GetProcAddress(fbUtilsHlib, 'FBSetUserName');
    @FBGetUserName          := GetProcAddress(fbUtilsHlib, 'FBGetUserName');
    @FBSetPassword          := GetProcAddress(fbUtilsHlib, 'FBSetPassword');
    @FBGetPassword          := GetProcAddress(fbUtilsHlib, 'FBGetPassword');
    @FBRecomputeIndexStatistics := GetProcAddress(fbUtilsHlib, 'FBRecomputeIndexStatistics');
    @FBGenID                := GetProcAddress(fbUtilsHlib, 'FBGenID');
    @FBExecuteBlockFunc     := GetProcAddress(fbUtilsHlib, 'FBExecuteBlockFunc');
    @FBExecuteBlockProc     := GetProcAddress(fbUtilsHlib, 'FBExecuteBlockProc');
    @FBClearTable           := GetProcAddress(fbUtilsHlib, 'FBClearTable');

    {Загружаем адреса функций, экспортируемых в fbUtilsPool.pas}
    @FBPoolAddConnectionProfile         := GetProcAddress(fbUtilsHlib, 'FBPoolAddConnectionProfile');
    @FBPoolAddDefaultConnectionProfile  := GetProcAddress(fbUtilsHlib, 'FBPoolAddDefaultConnectionProfile');
    @FBPoolGetConnectionByProfile       := GetProcAddress(fbUtilsHlib, 'FBPoolGetConnectionByProfile');
    @FBPoolGetConnectionByParams        := GetProcAddress(fbUtilsHlib, 'FBPoolGetConnectionByParams');
    @FBPoolGetDefaultConnection         := GetProcAddress(fbUtilsHlib, 'FBPoolGetDefaultConnection');
    @FBPoolReturnConnection             := GetProcAddress(fbUtilsHlib, 'FBPoolReturnConnection');
    @FBPoolGetSize                      := GetProcAddress(fbUtilsHlib, 'FBPoolGetSize');

    {Загружаем адреса функций, экспортируемых в fbUtilsBackupRestore.pas}
    @FBBackupDatabaseOnServer                := GetProcAddress(fbUtilsHlib, 'FBBackupDatabaseOnServer');
    @FBBackupDatabaseAndCopyFromServer       := GetProcAddress(fbUtilsHlib, 'FBBackupDatabaseAndCopyFromServer');
    @FBRestoreDatabaseOnServer               := GetProcAddress(fbUtilsHlib, 'FBRestoreDatabaseOnServer');
    @FBCopyBackupToServerAndRestoreDatabase  := GetProcAddress(fbUtilsHlib, 'FBCopyBackupToServerAndRestoreDatabase');

    {Загружаем адреса функций, экспортируемых в fbUtilsIniFiles.pas}
    @FBCreateIniFile  := GetProcAddress(fbUtilsHlib, 'FBCreateIniFile');
    @FBFreeIniFile    := GetProcAddress(fbUtilsHlib, 'FBFreeIniFile');

    {Загружаем адреса функций, экспортируемых в fbUtilsDBStruct.pas}
    @FBCreateDataBaseDesc := GetProcAddress(fbUtilsHlib, 'FBCreateDataBaseDesc');
    @FBFreeDataBaseDesc   := GetProcAddress(fbUtilsHlib, 'FBFreeDataBaseDesc');

    {Загружаем адреса функций, экспортируемых в fbUtilsCheckDBStruct.pas}
    @FBCheckDBStruct := GetProcAddress(fbUtilsHlib, 'FBCheckDBStruct');
  end;
