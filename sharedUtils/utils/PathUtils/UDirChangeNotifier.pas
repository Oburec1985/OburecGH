...
type
  ...
  // ðàáîòàåò òîëüêî ñ îäíèì êàòàëîãîì???
  ...
  protected
    ...
    // âûïîëíÿåòñÿ â îñíîâíîì ïîòîêå
    procedure DoChange;
  public
    constructor Create(const ADirectory: WideString;WantedNotifications: TDirChangeNotifications);overload;
    constructor Create(const ADirectory: WideString;WantedNotifications: TDirChangeNotifications; syncmainthread:boolean);overload;
...
constructor TDirChangeNotifier.Create(const ADirectory: WideString;WantedNotifications: TDirChangeNotifications;
                               syncmainthread:boolean);
begin
  Create(ADirectory, WantedNotifications);
  fsyncProc:=syncmainthread;
end;
...
begin
  ...
  begin
    ...
    ReadDirectoryChangesW(FDirHandle, @Buffer, SizeOf(Buffer), True, NotifyFilter, nil, @FOverlapped, nil);

    {>> Attente de qqch ou de la fin }
    if WaitForMultipleObjects(2, @WaitHandles, False, INFINITE) = WAIT_OBJECT_0 then
      Break;
    ...
    repeat
      ...
      if (Action <> FILE_ACTION_RENAMED_OLD_NAME) and (FAction in FNotifList) then
      begin
        ///////////////// Oburec ////////////////
        if fSyncProc then
